...
H = Figure(size=(400*length(UU)+100,900),figure_padding=1)
axs_UP = [Axis(
	H[1,i],
	xlabel = L"$\delta$",
	ylabel = L"$V$"
...
for (i,U) in enumerate(UU)
	...
	if U==0.0
		text!(ax_H,0.05,6.2,text=L"$\mathrm{N}_y$",align=(:center,:top),color=:white)
	...
	elseif U==12.0
		text!(ax_H,0.1,6.2,text=L"$\mathrm{N}_y$",align=(:center,:top),color=:white)
		heatmap!(ax_F,xx,yy,mask,colormap=colorschemes[:tabcoolrev])

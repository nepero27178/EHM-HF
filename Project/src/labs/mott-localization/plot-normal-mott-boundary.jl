...
# FilePathOut = LAB_ROOT * "/normal-mott-boundary-tS.pdf"
...
# ax = H.content[1]
# text!(ax,0.1,7.0,text="Mott localisation",color=tabblue,align=(:center,:center))
...
P = Plot3D(FilePathIn;Print=true,xVar="δ",yVar="V",zVar="f",cs=:tabquiet)
H = P[1].H
df = filter(:V => x -> x==0.0, P[1].DF)
xx = vcat(-0.05,df.δ)
...
# ax = H.content[1]
# text!(ax,0.1,7.0,text="Mott localisation",color=:white,align=(:center,:center))

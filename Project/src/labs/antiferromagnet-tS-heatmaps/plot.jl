...
Setup = "C[128]"
FilePathIn = "/home/nepero27178/Thesis/EHM-HF/Project/data/refined/Mode=rs/Setup=$(Setup)/Phase=AF/RB=S_Syms=.csv"
zVar = "uS"
cs=colorschemes[:tabcoolerrev]

FilePathOut = LAB_ROOT * "/af-$(zVar)-data-$(Setup).pdf"
...
text!(ax_H,7.9,0.25,text="Soft shrinking",color=:white,align=(:right,:bottom))
text!(ax_H,0.1,7.75,text="Hard shrinking",color=:black,align=(:right,:top),rotation=pi/2)

ax_C = Axis(H[1,2])
ax_C.xlabel = ax_P.xlabel.val
ax_C.xticks = ax_P.xticks.val
ax_C.yticks = ([2,4,4.93,6,8],["2","4","4.93","6","8"])

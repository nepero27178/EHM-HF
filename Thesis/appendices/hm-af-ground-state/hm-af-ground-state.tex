...
In this appendix we give some details about the antiferromagnetic phase in the conventional Hubbard model of Eq.~\eqref{eq:hubbard-model},
\begin{equation}\label{appeq:hm}
	...
	\text{where $t, U  > 0$,}
\end{equation}
approximated by HF methods. We use the results discussed in Sec.~\ref{sec:af-hm}.

\section{The AF ground-state for the conventional HM}
...
\]
where we defined the pseudospin operators and the pseudofield, see Eqs.~\eqref{eq:af-pseudospin-def}, \eqref{eq:af-pseudofield-def},
\[
...
From Eqs.~\eqref{eq:af-hm-id-def} and \eqref{eq:af-hm-z-psi},
\[
...
It follows that composing Eqs.~\eqref{appeq:af-hm-gs-1} and \eqref{appeq:af-hm-tilted-down} the ground state $\ket{\Psi}$ can be written as
...
\end{equation}
having we absorbed in the relative phase $e^{2i\zeta_{\mathbf{k}\sigma}}$ the two states phases, as well as the original sign difference of Eq.~\eqref{appeq:af-hm-tilted-down}. Now, inserting the explicit form of the ground state from equation above:
\[
\begin{aligned}
...
\[
	...
	k\in\mathbb{Z} \, .
\]
Using Eq.~\eqref{appeq:af-hm-gs-2}, we conclude
...
Finally, using Eq.~\eqref{eq:af-Gamma=WPsi}, we check that correctly the ground-state describes a perfect degenerate Fermi gas described by operators $\hat \gamma^{(-)}_{\mathbf{k}\sigma}$. Indeed,
\[
\begin{aligned}
	\hat \gamma^{(-)}_{\mathbf{k}\sigma} &= \left[W_{\mathbf{k}\sigma} \hat \Psi_{\mathbf{k}\sigma}\right]_2 \\
	&= \sin \theta_{\mathbf{k}\sigma} \hat c_{\mathbf{k}\sigma} + \cos \theta_{\mathbf{k}\sigma} \hat c_{\mathbf{k}+\bm{\pi}\sigma}
\end{aligned}
\]
Therefore, Eq.~\eqref{appeq:af-hm-gs-3} reduces to
...
Now, as we specified in Sec.~\ref{sec:af-phase-theory}, commensurate antiferromagnetism is described by the simultaneous SSBs
\[
\begin{aligned}
...
\]
with $\bm{0}$ the $2\times2$ zero matrix. Diagonalising $\bm{\chi}_0^{+-}(\mathbf{q},0)$ (which is symmetric due to sublattice exchange symmetry) we can express the above equation in terms of the diagonal matrix containing its eigenvalues.
...
\end{align}
being $\epsilon_\mathrm{F}$ the Fermi energy and $N(\epsilon)$ the DoS at energy $\epsilon$. The reason for the distinction above is the intrinsic metallic nature of the system for infinitesimal doping. As we discuss at the end of Sec.~\ref{subsec:af-hm-diagonalisation}, the half-filled AF phase is insulating, because the chemical potential lies within the gap. For an insulator Fermi energy is not defined. Instead, at finite filling the phase is metallic because the chemical potential crosses the upper band.

From Eq.~\eqref{appeq:af-instability-condition} and \eqref{appeq:af-unstable-half}, we see that the half filled antiferromagnet is stable towards transverse spin fluctuations, because $\chi^{+-}$ develops a divergence at zero frequency. At any finite doping, since we are describing a metal, a Fermi surface exists and thus $N(\epsilon_\mathrm{F})$ is finite. This makes the phase unstable, and this is the reason we do not discuss the AF phase outside of half-filling.

As we discuss at the beginning of Chap.~\ref{chap:af}, antiferromagnetism can establish in many ways. For instance, striped antiferromagnets are of particular interest \cite{wietek2021stripes}. In order to investigate the most stable physical realisation of an antiferromagnet on the conventional HM, on a pure HF level, we should implement a variational search for the minimum of free energy while varying the AF wavevector $\mathbf{k}$. Notice that at incommensurate wavevectors, $\mathbf{k} \neq \bm{\pi}$, the unit cell is not as simple as a pair of sites, but can become much larger. Hence, the HF analytical solution would be more complicated than our pseudospin representation of the HM hamiltonian in the commensurate AF channel.
...
In Fig.~\ref{appfig:mdU-beta=100} magnetisation $m$ at $\beta=100$ is plotted. As is visible in Fig.~\ref{appsubfig:mU-beta=100}, ignoring instability issues, doping disrupts AF ordering; the horizontal asymptote lowers as $\delta$ gets bigger. Also, for $\delta > 0$, a finite magnetisation $m$ exists only for $U \ge U_\mathrm{c}[\delta]$ (a critical value parametrically dependent on doping). In Fig.~\ref{appsubfig:dU-beta=100}, we show the dependence of the magnetisation on doping varying the local repulsion $U$. As $U$ grows, the maximum value of $\delta$ at which a finite magnetisation is developed gets larger.
...
\begin{figure}
	...
	\caption{Plots of $m$ at low-temperature as a function of both the local repulsion $U/t$ (left) and the doping $\delta = n-1/2$ (right).}
	\label{appfig:mdU-beta=100}
\end{figure}


\section{Analytic expression of self-consistent magnetisation at half-filling}

In this subsection, we provide an analytic expression to compute $m$ self-consistently at half-filling. Let us multiply Eq.~\eqref{eq:af-hm-m-self-consistency} on both sides by $U$. Since at half filling $\mu=0$, we have the finite-temperature self-consistent equation
\[
...
\]
being $\rho(\epsilon)$ the $2$D square lattice DoS. Its explicit form \cite{kogan2021greens} is given by
\[
...
\]
where $K(x)$ is the complete elliptic integral of the first kind, shown in Fig.~\ref{appsubfig:K(x)}.
%
\begin{figure}
	\centering
	...
	\caption{Plots of the complete elliptic integral of the first kind $K(x)$ and the DoS for the planar square lattice.}
	\label{appfig:square-dos}
\end{figure}
%
As is plotted in Fig.~\ref{appsubfig:D(x)}, the DoS $\rho(\epsilon)$ is logarithmically peaked at $\epsilon=0$, on the MBZ. A common approximation is to substitute the actual DoS with a step function,
\[
...
\]
thus
\begin{equation}\label{appeq:m-analytic}
	m \simeq \frac{\epsilon_0}{U\sinh(1/\rho_0 U)} \, ,
\end{equation}
having we used $\Delta = mU$. In the left panel of Fig.~\ref{appfig:m-analytic} we sketch this function of $U$ varying $\rho_0$, $\epsilon_0$. The same data of Fig.~\ref{appsubfig:mU-beta=100}, $\delta=0$ are plotted as black dots. The shown curves are not fits.

\begin{figure}
	\centering
	\includegraphics[height=0.32\linewidth]{../Project/src/labs/m-analytic/m-analytic.pdf}
	\caption{Sketch of the analytic expression for $m$, compared with the numerical results (black dots). The left panel shows a family of coloured curves described by Eq.~\eqref{appeq:m-analytic}. The right panel shows a family of coloured curves described by Eq.~\eqref{appeq:m-approx}, which approximates $m$ only at $U \to 0$.}
	\label{appfig:m-analytic}
\end{figure}
...
\begin{align}
	...
	&\simeq \lim_{U\to 0} \frac{2\epsilon_0}{U} \exp(-\frac{1}{\rho_0 U}) \, , \label{appeq:m-approx} 
\end{align}
giving a near-$0$ exponential suppression of magnetisation, typical of MFT self-consistent solutions. In the right panel of Fig.~\ref{appfig:m-analytic} we sketch this approximation.
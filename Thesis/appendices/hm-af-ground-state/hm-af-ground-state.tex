...
\begin{equation}\label{appeq:hm}
	...
	t, U  > 0 \, ,
\end{equation}
approximated by HF methods. We use the results discussed in Sec.~\ref{subsec:af-features}.
...
As is plotted in Fig.~\ref{appsubfig:D(x)}, the resulting DoS is logarithmically peaked at $\epsilon=0$, on the MBZ. A common approximation is to substitute the actual DoS with a step function,
\[
...
\]
Suppose the system is magnetised, $\Delta>0$. At low temperature, $\beta \gg 1$, as a first approximation we can substitute the hyperbolic tangent with unity, obtaining
\[
\begin{aligned}
...
\end{equation}
having we used $\Delta = mU$. In the left panel of Fig.~\ref{appfig:m-analytic} we sketch this function of $U$ varying $\rho_0$, $\epsilon_0$. The same data of Fig.~\ref{appsubfig:mU-beta=100}, $\delta=0$ are plotted as gray dots. The shown curves are not fits.

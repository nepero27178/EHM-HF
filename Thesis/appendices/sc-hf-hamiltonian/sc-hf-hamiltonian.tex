...
\end{equation}
The above equation ``completes'' Eq.~\eqref{eq:sc-<cc>}, and completely specifies the behaviour of fermionic EVs in the translationally invariant SC phase.
...
\end{bmatrix} \, ,
\]
with $\tau^\alpha$ the Pauli matrices of Eq.~\eqref{eq:pauli-def}. As was for the AF phase, the pseudospin operators $\hat s_\mathbf{k}^\alpha$ satisfy a $\mathfrak{su}(2)$ algebra up to a constant, irrelevant in the context of this discussion. They satisfy the identities:
\begin{align}
...
\begin{tabproof}[Proof of Eq.~\eqref{appeq:sc-hm-ws=0}: impossibility of $s$-wave superconductivity in the conventional HM]
	...
	\begin{equation}\label{eq:sc-hm-Uc}
		...
		\right]^{-1} \, .
	\end{equation}
	By construction $U_\mathrm{c}[\beta] > 0$. Using

...
In this appendix, we derive the analytic expression of the HF hamiltonian for the SC phase, demonstrating all the results mentioned in Sec.~\ref{sec:sc-hf-hfps}. In particular, we aim to obtain Eqs.~\eqref{eq:sc-H-approx} and \eqref{eq:sc-Ec}. We start by discussing the SC phase on the conventional HM, and then generalise to the EHM.
...
\end{align}
Note that translational invariance is unbroken. Therefore, Eqs.~\eqref{eq:normal-<cc>} and \eqref{eq:normal-<n_i>}, derived in the context of the normal phase, hold equivalently,
\begin{align}
...
\section{Superconductivity in the conventional HM: derivation of the HF hamiltonian}\label{sec:sc-hm}
...
Following Tab.~\ref{tab:HU-sel-rules} and Eq.~\eqref{eq:mft-HU}, we know that the local repulsion decomposes in Hartree and Cooper channels,
\[
\begin{aligned}
...
\subsection{Effects of the local Hartree terms: chemical potential renormalisation}

For the Hartree channel, being the density uniform across sites, we can re-use the entirety of Sec.~\ref{subsec:normal-hartree-mu} for the normal phase. Using Eq.~\eqref{eq:sc-<n_i>}, we get the approximation of Eq.~\eqref{eq:normal-HU-hartree-approx},
...
\subsection{Effects of the local Cooper terms: isotropic pairing}
...
\end{equation}
The above equation is similar to Eq.~\eqref{eq:sc-<cc>}.
...
\end{bmatrix} \, ,
\]
with $\tau^\alpha$ the Pauli matrices of Eq.~\eqref{eq:pauli-def}. As was for the AF phase in Eq.~\eqref{eq:af-almost-su2}, the pseudospin operators $\hat s_\mathbf{k}^\alpha$ satisfy a $\mathfrak{su}(2)$ algebra up to a constant, irrelevant in the context of this discussion. They satisfy the identities:
\begin{align}
...
\end{equation}
Recall the Bogoliubov bands for the AF phase, of Eq.~\eqref{eq:af-bands}. The key difference, here, is that the bands $E_\mathbf{k}$ depend on $\xi_\mathbf{k}$, not $\epsilon_\mathbf{k}$. We discussed at the end of Sec.~\ref{subsec:af-hm-diagonalisation} that, in the AF phase, the chemical potential stays the same for electrons and Bogoliubov particles. For the SC phase, being the bands $E_\mathbf{k}$ dependent on $\tilde{\mu}$ through $\xi_\mathbf{k}$, the chemical potential for the Bogoliubov particles is identically zero. This implies that the chemical potential lies within the SC gap.
...
\end{align}
The proofs are identical to the ones provided for Eqs.~\eqref{eq:af-Th+=1}, \eqref{eq:af-Th-=tanh}. 
...
\subsection{Impossibility of $s$-wave superconductivity in the HM}\label{appsubsec:sc-w^s=0}
...
\begin{tabproof}[Proof of Eq.~\eqref{appeq:sc-hm-ws=0}: impossibility of $s$-wave superconductivity in the conventional HM]
	...
	\begin{equation}\label{eq:sc-hm-Uc}
		...
		\right]^{-1} \, .
	\end{equation}
	By construction $U_0 > 0$ at any choice of $\beta$. Using
	Eq.~\eqref{eq:sc-hm-ws-self-consistency-2} and $\Delta_\mathbf{k} = -U w^{(s)}$, Eq.~\eqref{eq:sc-hm-Uc} can be rewritten as:
	\[
	w^{(s)} = -\frac{U}{U_0} w^{(s)} \, .
	\]
...
In this section we add to the hamiltonian the non-local attraction $\hat H_V$. As we did for the AF phase in Sec.~\ref{sec:af-ehm}, we look for a solution with an identical mathematical structure as the one here described for the conventional HM. We define the renormalised quantities
\[
...
\subsection{Effects of the non-local Hartree terms: chemical potential renormalisation}\label{subsec:sc-hartree-mu}
...
\end{multline*}
where we used Eq.~\eqref{eq:mft-HV}. We have already carried out this computation in Sec.~\ref{subsec:normal-hartree-mu}, which led to the renormalisation of the chemical potential. The non-local attraction $\hat H_V$ is reduced in the Hartree channel to
...
\end{equation}
This, together with Eq.~\eqref{eq:sc-HU-hartree-approx}, gives the chemical potential full renormalisation, identical to Eq.~\eqref{eq:normal-mu-shift},
...
\subsection{Effects of the Fock terms: bare bands renormalisation}\label{subsec:sc-fock}
...
\end{equation}
From now on we omit spin index, using spin rotational invariance. Each harmonic $u^{(\gamma)}$ satisfies the self-consistency Eq.~\eqref{appeq:sc-ugamma-harmonics}, which is written identically as:
...
\begin{table}
	...
	\begin{tabular}{r E l l}
		...
		$d$-wave & $u^{(d)}$ & $\displaystyle \frac{1}{2L_x L_y} \sum_{\mathbf{k}\in\mathrm{BZ}} (\cos k_x - \cos k_y) \left[
		1 - \frac{\tilde{\xi}_\mathbf{k}}{\tilde{E}_\mathbf{k}} \tanh(\frac{\beta \tilde{E}_\mathbf{k}}{2})
		\right] $ & Fig.~\ref{subfig:d-wave-correlator} \\
		\midrule
	\end{tabular}
	\caption{Symmetry resolved self-consistency equations for the harmonics of $u_\mathbf{k}$. The shown equations are reformulations of Eq.~\eqref{appeq:sc-ugamma-harmonics}, which is equal to Eq.~\eqref{eq:sc-hm-ugamma-self-consistency}.}
...
\begin{tabproof}[Proof of Eq.~ \eqref{eq:sc-hm-ugamma-self-consistency}: self-consistency equations for $u^{(\gamma)}$]
	...
	\end{align}
	having we used Eq.~\eqref{eq:sc-~Th} and sequent. Inserting this result in Eq.~\eqref{appeq:sc-ugamma-harmonics}, we get Eq.~\eqref{eq:sc-hm-ugamma-self-consistency} in this spin sector. 
...
\subsection{Effects of the non-local Cooper terms: generalised pairing}\label{subsec:sc-cooper}
...
\end{equation}
In both the above equations, the sum is intended for $\gamma \in \lbrace s^*, p_x, p_y, d \rbrace$.
...
\begin{tabproof}[Proof of Eq.~\eqref{eq:sc-HV-cooper-approx}: HF approximation of $\hat H_V$ in Cooper channel]
	...
	\end{multline}
	Sum is intended for $\gamma \in \lbrace s^*, p_x, p_y, d \rbrace$. We recognised $E_{\mathrm{C},V}^{(\mathrm{SC})}$ as defined in Eq.~\eqref{eq:sc-HV-cooper-Ec-implicit}.  Eq.~\eqref{eq:sc-V-os-cooper-int1} is reduced to
	\begin{multline} 
		\hat H_V^{(\mathrm{o.s.})} \HFsimeq \text{(Hartree channel)}
		-\sum_{\gamma\neq s}
		\left(
...
The contraction energy of Eq.~\eqref{eq:sc-HV-cooper-Ec-implicit} can be written as
\begin{equation}\label{eq:sc-HV-cooper-Ec}
	E_{\mathrm{C},V}^{(\mathrm{SC})} = - L_x L_y \times V \sum_{\gamma\neq s} \big| w^{(\gamma)} \big|^2 \, ,
\end{equation}
with $\gamma \in \lbrace s^*, p_x, p_y, d \rbrace$.
...
Each harmonic, including $s$-wave, satisfies the self-consistency Eq.~\eqref{appeq:sc-wgamma-harmonics}, which is written equivalently as
...
\begin{table}
	...
	\begin{tabular}{r E l l}
		...
		$d$-wave & $w^{(d)}$ & $\displaystyle \frac{1}{2L_x L_y} \sum_{\mathbf{k}\in\mathrm{BZ}} (\cos k_x - \cos k_y) \frac{\tilde{\Delta}_\mathbf{k}}{\tilde{E}_\mathbf{k}} \tanh(\frac{\beta\tilde{E}_\mathbf{k}}{2}) $ & Fig.~\ref{subfig:d-wave-correlator} \\
		\midrule
	\end{tabular}
	\caption{Symmetry resolved self-consistency equations for the harmonics of $w_\mathbf{k}$. The shown equations are reformulations of Eq.~\eqref{appeq:sc-wgamma-harmonics}, which is equal to Eq.~\eqref{eq:sc-ehm-wgamma-self-consistency}.}
...
\begin{equation}\label{appeq:sc-H-approx}
...
\end{equation}
which coincides with Eq.~\eqref{eq:sc-H-approx}.
We included the anticommutation energy defined in Eq.~\eqref{appeq:sc-anticommutation-energy} and extended to renormalised quantities,
\[
	E_\mathrm{a}^{(\mathrm{SC})} \equiv \sum_{\mathbf{k}\in\mathrm{BZ}} \tilde{\xi}_\mathbf{k} \, ,
\]
and the total contraction energy $E_\mathrm{c}^{(\mathrm{SC})}$, which includes all the shifts to energy we encountered. These are given, respectively, in Eqs.~\eqref{eq:sc-HU-hartree-Ec}, \eqref{eq:sc-HU-cooper-Ec}, \eqref{eq:sc-HV-hartree-Ec}, \eqref{eq:sc-HV-fock-Ec} and \eqref{eq:sc-HV-cooper-Ec}. Namely,
\begin{align}\label{appeq:sc-Ec}
	...
	\right) \, ,
\end{align}
which coincides with Eq.~\eqref{eq:sc-Ec}.
...
\end{equation}
which coincides with Eq.~\eqref{eq:sc-ehm-full-bands}. The full gap function, as in Eq.~\eqref{eq:sc-ehm-full-gap}, is given by
...
\]
In Sec.~\ref{sec:sc-numeric}, we investigate the competition among these components.
...
\[
	...
	\right) \, .
\]
These concept are summaries in Tab.~\ref{tab:sc-ssc-tsc-hfps}, where we list the HFPs in singlet and triplet channels.
...
\subsection{The energetic constraint on the gap harmonics}\label{appsubsec:sc-w^s-constraint}

In this final section we present a proof of the constraint of Eq.~\eqref{eq:sc-wgamma-circle}. We derived the expression of Eq.~\eqref{eq:sc-fMFT}, which contains in its second row the HFPs $u^{(\gamma)}$ and $w^{(\gamma)}$. We have:
...
\]
We recognise Eq.~\eqref{eq:sc-wgamma-circle}. For the conventional HM, we know from Sec.~\ref{subsec:sc-hm-diagonalisation} that $w^{(s)}=0$. For the EHM, it can assume non-zero values, as long as Eq.~\eqref{appeq:sc-wgamma-circle} is satisfied.

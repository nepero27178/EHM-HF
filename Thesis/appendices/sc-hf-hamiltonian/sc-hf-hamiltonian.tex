...
\begin{equation}\label{appeq:sc-Ec}
	...
	\right)
\end{equation}
which include all the shifts to energy we encountered. These are given, respectively, in Eqs.~\eqref{eq:sc-HU-hartree-Ec}, \eqref{eq:sc-HU-cooper-Ec}, \eqref{eq:sc-HV-hartree-Ec}, \eqref{eq:sc-HV-fock-Ec} and \eqref{eq:sc-HV-cooper-Ec}. Eq.~\eqref{appeq:sc-Ec} coincides with Eq.~\eqref{eq:sc-Ec}.
...
\]
For the conventional HM, we know from Sec.~\ref{subsec:sc-hm-diagonalisation} that $w^{(s)}=0$. For the EHM, it can assume non-zero values, as long as Eq.~\eqref{appeq:sc-wgamma-circle} is satisfied.

...
\end{equation}
We work with gaussian states. Thus Eq.~\eqref{eq:hf-wick-hfc} holds exactly (adapted for two indices instead of four),
\[
\alpha \neq \beta
\quad\colon\quad
\langle
...
\]
Applying Eqs.~\eqref{appeq:mft-x-implicit} and \eqref{appeq:mft-y-implicit}, we get the identities
\begin{align}
...
\end{align}
These equations are a rephrasing of Eqs.~\eqref{eq:hf-solution-h+-}, \eqref{eq:hf-solution-h++} for the fields $h_{\alpha\beta}^{pq}$, setting $A_{\alpha\beta}=0$. This can be checked by using Eq.~\eqref{appeq:mft-B=2V}.

Now, consider the special case where $\forall \alpha \colon V_{\alpha\alpha}=0$. This is the case for the EHM, see Eq.~\eqref{eq:extended-hubbard-model}. Then the above equations imply:
\begin{multline*}

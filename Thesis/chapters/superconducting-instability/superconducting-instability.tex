...
\end{align}
The proof is identical to the one thereby provided.
...
\end{enumerate}
This concept is summarised in Tab.~\ref{tab:sc-pairings-symmetry-structures}. The HF hamiltonian will be expressed in terms of a set of HFPs which are linked to EVs of operators that create a Cooper pair. Hence, the symmetry properties of Cooper pairs are determined by those of the HF hamiltonian.

When dealing superconductivity, we can choose to restrict to just one of these pairing channels. In terms of the HF method, this means that we limit the variational search to the subset of states that are expressed in terms of pairs bound via a Cooper mechanism in the singlet or triplet channel. These limitations will provide specific symmetry selection rules on the superconducting gap function harmonics. In this thesis, we take on computations in general and in the end restrict to the singlet channel.
...
\end{align}
As was for the AF phase, these harmonics are HFPs in the SC phase and must be determined self-consistently. Eqs.~\eqref{eq:sc-ugamma-harmonics} and \eqref{eq:sc-wgamma-harmonics} are their self-consistency equations.
...
In the Hartree channel, $\hat H_U$ is approximated as
...
\begin{tabproof}[Proof of Eq.~\eqref{eq:sc-HU-cooper-approx}: HF approximation of $\hat H_U$ in Cooper channel]
	...
	Using Eq.~\eqref{eq:sc-ehm-phi-def}, Eq.~\eqref{eq:sc-mft-HU} and the fact that Cooper pairs have zero net momentum, we obtain
	\begin{equation}\label{eq:sc-HU-<phi>-phi}
		...
		\right) - E_{\mathrm{C},U}^{(\mathrm{SC})} \, ,
	\end{equation}
	Using Eq.~\eqref{eq:sc-wq-def}, we have
	\begin{multline*}
	...
	\end{multline*}
	Using Eq.~\eqref{eq:sc-wgamma-harmonics}, we see that the parenthesised terms coincide, respectively, with $w^{(s)}$ and $w^{(s)*}$. Applying Eq.~\eqref{eq:sc-Delta^s}, we obtain Eq.~\eqref{eq:sc-HU-cooper-approx}.	
\end{tabproof}
...
\begin{tabproof}[Proof of Eq.~\eqref{eq:sc-HU-cooper-Ec}: local Cooper contraction energy.]
	...
	Consider Eq.~\eqref{eq:sc-HU-hartree-Ec-implicit}. Substituting Eqns.~\eqref{eq:sc-wq-def}, \eqref{eq:sc-wgamma-harmonics} we get:
	\begin{align}
...
\begin{tabproof}[Proof of Eq.~\eqref{eq:sc-HM-hk}]
	...
	Summing Eq.~\eqref{eq:Ht-Fourier} and Eq.~\eqref{eq:sc-HU-cooper-approx}, we obtain the approximated hamiltonian
	\begin{multline}
	...
	\end{multline}
	Due to inversion symmetry, it holds $\xi_\mathbf{k} = \xi_{-\mathbf{k}}$. Using Fermi anticommutation rules,
	\[
		\acomm{\hat c_\alpha}{\hat c_\beta} = \delta_{\alpha\beta} \, ,
	\]
	...
	\begin{multline*}
		...
		\right) \, ,
	\end{multline*}
	with $E_{\mathrm{H},U}^{(\mathrm{SC})}$, $E_{\mathrm{C},U}^{(\mathrm{SC})}$ given respectively in Eqns.~\eqref{eq:sc-hartree-U-contraction-energy} and \eqref{eq:sc-cooper-U-contraction-energy}- We substituted $\Delta^{(s)} = \Delta_\mathbf{k}$. Using the Nambu spinors defined in Eq.~\eqref{eq:sc-Phi-def} and the $2 \times 2$ matrix $h_\mathbf{k}$ of Eq.~\eqref{eq:sc-HM-hk}, the HM hamiltonian is reduced to the matrix form of Eq.~\eqref{eq:sc-HM-hk}.
\end{tabproof}
...
In general, $\Delta^{(s)} = -U \langle \hat \phi_\mathbf{k}^\dagger \rangle \in \mathbb{C}$. However, we can always choose $\Delta^{(s)}$ as purely real. This is due to the fact that the hamiltonian is gauge-invariant. Indeed, if we set in Eq.~\eqref{eq:gauge-symmetry}
\begin{equation}\label{eq:sc-gauge-ws-real}
	\eta \equiv -\frac{\arg w^{(s)}}{2} \, ,
\end{equation}
we have $\Delta^{(s)} \in \mathbb{R}$.

\begin{tabproof}[Setting the gauge of Eq.~\eqref{eq:sc-gauge-ws-real}, we obtain a real gap]
	...
	\]
	which implies that $w^{(s)}$ is purely real under this gauge transform.	
\end{tabproof}
...
\end{align}
We notice that, as is expressed in Tab.~\ref{tab:sc-pseudo-spin-map}, the $z$ component of the pseudospin is related to density, being $1+\ev{\hat s_\mathbf{k}^z} = \ev{\hat n_{\mathbf{k}\uparrow}	+ \hat n_{-\mathbf{k}\downarrow}}$.
...
\begin{equation}\label{eq:sc-hm-ws-self-consistency-2}
	w^{(s)} = \frac{1}{2 L_x L_y} \sum_{\mathbf{k}\in\mathrm{BZ}} \frac{\Delta_\mathbf{k}}{E_\mathbf{k}} \mathrm{Th}_\mathbf{k}^{(-)}[\beta] \, .
\end{equation}
...
\begin{tabproof}[Proof of Eq.~\eqref{eq:sc-hm-ws-self-consistency-2}: self-consistency equation for $w^{(s)}$]
	...
	\]
	From Eq.~\eqref{eq:sc-wgamma-harmonics}, we obtain
	\[
	\begin{aligned}
...
\]
Angles are changed too. Let $\tilde{\theta}_\mathbf{k}$ be defined through the relations
...
\end{equation}
Using these definition, we compute the pseudospin EVs
\begin{align}
...
Renormalisation of thermal factors and $z$ pseudospin EV in tilted frame gives:
\begin{align}
	\langle \hat \Gamma_\mathbf{k}^\dagger \tau^z \hat \Gamma_\mathbf{k} \rangle = 
	f\left(
...
\end{align}
The proof is identical to the one provided for Eqs.~\eqref{eq:af-Th+=1}, \eqref{eq:af-Th-=tanh}, with the difference that now Eqs.~\eqref{eq:sc-Th+=1}, \eqref{eq:sc-Th-=tanh} hold at any value of $\delta$.
...
We have set grounds for the search of the HF solution for the EHM. From Tab.~\ref{tab:HV-sel-rules} we know the symmetry selection rules for Hartree, Fock and Cooper channel for the non-local attraction. We break down each.
...
\begin{equation}\label{eq:sc-HV-fock-Ec}
	...
	\right) \, .
\end{equation}
The proofs are identical to those presented in Sec.~\ref{subsec:normal-fock-bands}, because the SC phase is translationally invariant as was the normal phase. 

As was for the normal and AF phases, bands are renormalised as
...
\begin{tabproof}[Proof of Eq.~\eqref{eq:sc-HV-cooper-approx}: HF approximation of $\hat H_V$ in Cooper channel]
	...
	\end{multline}
	Sum is intended for $\gamma \in \lbrace s^*, p_x, p_y, d_{x^2-y^2} \rbrace$. We recognised $E_{\mathrm{C},V}$ as defined in Eq.~\eqref{eq:sc-HV-cooper-Ec-implicit}.  Eq.~\eqref{eq:sc-V-os-cooper-int1} is reduced to
	\begin{multline} 
		\hat H_V^{(\mathrm{o.s.})} \HFsimeq \text{(Hartree channel)}
		-\sum_{\gamma\neq s}
		\left(
...
The contraction energy reads:
\begin{equation}\label{eq:sc-HV-cooper-Ec}
	E_{\mathrm{C},V}^{(\mathrm{SC})} = - L_x L_y \times V \sum_\gamma \big| w^{(\gamma)} \big|^2 \, .
\end{equation}
...
\begin{tabproof}[Proof of Eq.~\eqref{eq:sc-ehm-wgamma-self-consistency}: self-consistency equations for $w^{(\gamma)}$]
	...
	Consider Eq.~\eqref{eq:sc-wgamma-harmonics}. For the $\gamma$-wave component of the superconducting order parameter, the following chain, similar to Eq.~\eqref{eq:sc-hm-ws-self-consistency-2}, holds:
	\[
	\begin{aligned}
...
\begin{equation}\label{eq:sc-Ec}
	...
	\right)
\end{equation}
which include all the shifts to energy we encountered. These are given, respectively, in Eqs.~\eqref{eq:sc-HU-hartree-Ec}, \eqref{eq:sc-HU-cooper-Ec}, \eqref{eq:sc-HV-hartree-Ec}, \eqref{eq:sc-HV-fock-Ec} and \eqref{eq:sc-HV-cooper-Ec}. Note the minus sign in front of it.
...
\[
	...
	\right) \, .
\]
Before moving to the last part of this section, which gives an operative procedure to estimate the ground-state free energy and entropy densities, we dedicate a short section to the explicit form of the SC ground-state at zero temperature.
...
\end{aligned}
\]
These results are coherent with Eqns.~\eqref{eq:sc-x-expectation-non-local} as well as \eqref{eq:sc-y-expectation-non-local}. We identify the relative phase in the BCS state with angle of the pseudofield in the $xy$ plane.
...
\begin{tabproof}[Proof of Eq.~\eqref{eq:sc-s+=s-}: Bogoliubov bands $\pm \tilde{E}_\mathbf{k}$ give equal entropy densities.]
	...
	\[
		...
		\Big] \, ,
	\]
	therefore $s_-^{(\mathrm{SC})}=s_+^{(\mathrm{SC})}$.	
\end{tabproof}
%
...
\[
	\tilde{\Delta}^{(s)} = -U w^{(s)} \, .
\]
In the conventional HM we know that $s$-wave superconductivity is impossible at HF level. This was discussed in Sec.~\eqref{subsec:sc-features}. Instead in the EHM there is no symmetry selection rule for $w^{(s)}$. The only constraint comes from Eq.~\eqref{eq:sc-wgamma-circle}: $w^{(s)}$ can be non-zero as long as at least one $w^{(\gamma)}$ with $\gamma=s^*,d$ is non-zero. This is a necessary but insufficient condition.

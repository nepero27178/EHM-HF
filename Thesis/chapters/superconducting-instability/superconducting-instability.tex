...
This chapter is organised as follows. In Sec.~\ref{sec:sc-phase-theory}, we present the translationally invariant superconducting (SC) phase and its symmetries. In Sec.~\ref{sec:sc-hf-hfps}, we report the HF approximation to the EHM hamiltonian in the normal phase, the self-consistency equations and two constraints for the various HFPs.
In Sec.~\ref{sec:sc-numeric}, we present the results of numeric simulations.
...
\[
	\mathrm{U}^z(1) \SSB 0 \, , \\
\]
as we discussed in Sec.~\ref{sec:ehm-sym-break}. In the SC phase, we have
\[
	\big\langle
...
\end{enumerate}
This concept is summarised in Tab.~\ref{tab:sc-pairings-symmetry-structures}. We can choose to restrict to just one of these pairing channels. This provides specific symmetry selection rules on the SC gap function harmonics. In this thesis, we carry out generic computations and then restrict to the singlet channel.

\begin{table}
	\centering
	\begin{tabular}{c c}
		\textbf{Spatial structure of the gap function} & \textbf{Pairing channel} \\
		\midrule
		Inversion symmetric & Singlet pairing \\
		Inversion anti-symmetric & Triplet pairing \\
		\midrule
	\end{tabular}
	\caption{Relation between the spatial inversion (intended as $(x,y) \to (-x,-y)$) symmetry properties of the SC gap, and the pairing channel for Cooper pairs.}
	\label{tab:sc-pairings-symmetry-structures}
\end{table}

As we did for the normal and AF phases, we also allow for
\[
	\mathrm{C}_4 \SSB \mathrm{C}_2 \, .
\]
We will observe the breaking of planar rotational symmetry in the SC phase.

\section{The HF hamiltonian and the variational parameters}\label{sec:sc-hf-hfps}
...
\end{align}
The breaking of charge conservation, namely $\mathrm{U}^\mathrm{c}(1)$ symmetry, allows $w_\mathbf{k}$ to become non-zero. Instead, for a phase which preserves $\mathrm{U}^\mathrm{c}(1)$, $w_\mathbf{k}$ is identically zero.
...
\end{align}
As was for the AF phase, these harmonics are HFPs in the SC phase and must be determined self-consistently through Eqs.~\eqref{eq:sc-ugamma-harmonics} and \eqref{eq:sc-wgamma-harmonics}.
...
%	w^{(s)} = 0 \, ,
%\end{equation}
%therefore no SC gap can develop at HF level. This does not hold for $U<0$: an attractive model can host $s$-wave superconductivity.
%
...
%	
%	Consider Eq.~\eqref{eq:sc-wgamma-harmonics}. For the $\gamma$-wave component of the SC order parameter, the following chain holds:
%	\[
%	\begin{aligned}
...
%In this section, we derive one constraint on the solution for the SC phase, which ``regulates'' the competition among various symmetries of the SC gap. We specify that, differently from AF and normal phases, for the singlet SC phase we do not have any theoretical constraint on the HFPs $u^{(\gamma)}$, $w^{(\gamma)}$.
...
and the total contraction energy,
\begin{equation}\label{eq:sc-Ec}
	\frac{E_\mathrm{c}^{(\mathrm{SC})}}{L_x L_y} = -U \left(
	n^2 + \big|
	w^{(s)}
	\big|^2
	\right) + V \left(
...
\begin{table}
	...
	\begin{tabular}{c c}
		...
		\midrule
	\end{tabular}
	\caption{Set of HFPs for the two possible realisations of the SC phase, respectively with a symmetric and an antisymmetric SC gap.}
	\label{tab:sc-ssc-tsc-hfps}
\end{table}

As was for the normal and AF phase, and is reported in Eq.~\eqref{eq:sc-ehm-full-bands}, the occupation harmonics $u^{(\gamma)}$ are involved in the renormalisation of the bare bands. We report their self-consistency equations in explicit form, reformulating Eq.~\eqref{eq:sc-ugamma-harmonics}, in Tab.~\ref{tab:sc-ehm-ugamma-self-consistencies}. Instead, the HFPs $w^{(\gamma)}$ are components of the gap function, as we see in Eq.~\eqref{eq:sc-ehm-full-gap}. Indeed,
\[
	\tilde{\Delta}^{(s)} = -U w^{(s)} \, ,
	\qquad
	\tilde{\Delta}^{(\gamma)} = V w^{(\gamma)}
...
In this thesis, we focus on the singlet channel, therefore the first row of HFPs in Tab.~\ref{tab:sc-ssc-tsc-hfps}. It is possible to prove that:
\begin{itemize}
	...
	\begin{equation}\label{eq:sc-hm-ws=0}
		...
		w^{(s)} = 0 \, ,
	\end{equation}
	which implies that the SC gap is identically zero. This can be seen by plugging in Eq.~\eqref{eq:sc-ehm-full-gap} both $V=0$ and $w^{(s)}=0$. Eq.~\eqref{eq:sc-hm-ws=0} is proven in Sec.~\ref{appsubsec:sc-w^s=0}.
	%
...
We conclude this summary by recalling the renormalisation of the hopping parameter in the context of the normal phase. Being the SC phase translationally invariant, and since the band renormalisation of Eq.~\eqref{eq:sc-ehm-full-bands} is formally identical to the one given in Eq.~\eqref{eq:normal-bands-renormalisation}, the discussion of Sec.~\ref{subsec:normal-d-wave-hopping} holds identically, upon substituting in Eq.~\eqref{eq:normal-<nr>}
\[
...
\section{Numeric results for the the singlet SC phase}\label{sec:sc-numeric}
...
\begin{aligned}
	...
	t - \frac{u^{(s^*)}V}{2}
	\right) \varphi_{ij}^{(s^*)} \, , \\
	\tilde{t}_{ij}^{(d)} &\equiv - \frac{u^{(d)}V}{2} \varphi_{ij}^{(d)} \, , \\
	\tilde{\mu} &\equiv \mu + n (2zV-U) \, .
\end{aligned}
\]
We notice that, in contrast to the AF and normal phases, the renormalised hopping has a finite $d$-wave component. We ran simulations at $\beta=100.0$ (in absolute units, see Sec.~\ref{sec:normal-numeric}) within the boundaries:
\[
	0 \le U \le 12 \, ,
	\qquad
	0 \le V \le 8 \, ,
...
Consider the isotropic harmonic of the gap function, namely the $s$-wave component. In Fig.~\ref{fig:sc-data-ws-AB256-compared}, we show the data for $w^{(s)}$, which determines the isotropic gap through
\[
	\tilde{\Delta}^{(s)} = -U w^{(s)} \, .
\]
We now recall the constraint of Eq.~\eqref{eq:sc-hm-ws=0}. In the conventional HM, at HF level, isotropic superconductivity is impossible. As we move to the EHM, namely setting $V>0$, the SC gap can acquire a $s$-wave component, as long as the constraint of Eq.~\eqref{eq:sc-wgamma-circle} is satisfied: $w^{(s)}$ can be non-zero if at least one of  $w^{(s^*)}$, $w^{(d)}$ is non-zero. This is a necessary but not sufficient condition.

In Fig.~\ref{subfig:sc-data-ws-A256-compared} we see that $w^{(s)}$ is null at $\delta=0$ (left panel), namely
\begin{equation}\label{eq:sc-Delta^s=0}
	\tilde{\Delta}^{(s)}\big|_{\delta=0} = 0 \, ,
\end{equation}
within the range of parameters we explored. Increasing doping (central and right panels), $w^{(s)}$ is null up to a critical value $V_\mathrm{c}^{(s)}$, which depends also on $U$. For $V \ge V_\mathrm{c}^{(s)}$, $w^{(s)}$ is negative and the shape of the curve depends on $U$. In particular, the amplitude $\abs{w^{(s)}}$ exhibits a local maximum. We observe that doping lowers the critical value $V_\mathrm{c}^{(s)}$, with $V_\mathrm{c}^{(s)}/t \approx 1$, at almost unitary filling ($\delta = 0.4$). In Fig.~\ref{subfig:sc-data-ws-B256-compared}, we report the dependence of $w^{(s)}$ on doping. As anticipated, $w^{(s)}=0$ at half-filling. Varying $U/t$ and $V/t$ we see that $w^{(s)}=0$ reaches some negative minimum, whose absolute value lowers as $U$ grows.
...
In Fig.~\ref{fig:sc-data-wS-AB256-compared}, we show the results for the non-local isotropic HFP $w^{(s^*)}$. The resulting component of the full gap reads
\[
	\tilde{\Delta}^{(s^*)} = V w^{(s^*)} \, .
\]
Consider first Fig.~\ref{subfig:sc-data-wS-A256-compared}. At null and finite fillings, $w^{(s^*)}$ is null up to a critical value $V_\mathrm{c}^{(s^*)}$. Its position depends on $\delta$ and weakly on $U/t$. In particular, the half-filling curve (left panel) is independent of $U/t$. The local isotropic gap harmonic $\tilde{\Delta}^{(s)}$ was always null at half filling, see Eq.~\eqref{eq:sc-Delta^s=0}. In contrast, its non-local counterpart $\tilde{\Delta}^{(s^*)}$ is null up to the critical value delimiting the phase transition. Therefore, thanks to the non-local pairing, the SC gap at half-filling can develop a purely non-local isotropic component.

In Fig.~\ref{subfig:sc-data-wS-B256-compared}, we see that the dependence on doping of $w^{(s^*)}$ is peculiar. At low doping ($\delta \lesssim 0.3$), $w^{(s^*)}$ is small for small values of $V$ (left panel). At higher doping (central and right panels) we see that a local maximum emerges, indicating that $w^{(s^*)}$ converges to higher values at smaller values of $V$.
...
In Fig.~\ref{fig:sc-data-wd-AB256-compared}, we report our findings for the HFP $w^{(d)}$, which is related to the $d$-wave gap,
\[
	\tilde{\Delta}^{(d)} = V w^{(d)} \, .
\]
From the top panel (Fig.~\ref{subfig:sc-data-wd-A256-compared}), we see that $w^{(d)}$ depends weakly on $U$. At half-filling (left panel), the $d$-wave gap is zero only at $V=0$ for any value of $U$. Differently from the other components, $w^{(d)}$ grows to finite values already at infinitesimal $V/t$. Recalling the results of Figs.~\ref{fig:sc-data-ws-AB256-compared} and \ref{fig:sc-data-wS-AB256-compared}, we note that when the strength of the non-local attraction grows enough, the $s^*$-wave part sets in, meaning that, in this region, the SC gap has a mixed symmetry. At finite filling (central and right panels), the $d$-wave gap emerges for $V > V_\mathrm{c}^{(d)}$.

In the bottom row (Fig.~\ref{subfig:sc-data-wd-B256-compared}), we show the dependence of $w^{(d)}$ on doping at different values of $U$. For any value of $V/t$, the HFP $w^{(d)}$ is a monotonically decreasing function of $\delta$, and the critical value $V_\mathrm{c}^{(d)}$ increases with doping. Because of this, we see that the lighter curves of Fig.~\ref{subfig:sc-data-wd-B256-compared}, corresponding to lower values of $V$, reach zero quite rapidly for $\delta \gtrsim 0.3$. The curves for larger values of $V$, instead, reach zero at almost unitary filling. In all three panels of Fig.~\ref{subfig:sc-data-wd-B256-compared} we see that at doping $\delta \approx 0.2$ various lines cross at $w^{(d)} \approx 0.3$. Finally, at null doping, there exists a finite value of $V$ that maximises $w^{(d)}$, see Fig.~\ref{subfig:sc-data-wd-A256-compared}.

\paragraph{Competition among the channels.}

To understand the competition among channels, we compare the values of the harmonics $\tilde{\Delta}^{(\gamma)}$ for $\gamma=s,s^*,d$. In Fig.~\ref{fig:sc-data-surface-wS-B256}, we plot the different components as a function of $\delta$ and $V$ for $U/t=4$. Data and plots for other values of $U$ \href{https://github.com/nepero27178/EHM-HF}{are openly accessible at our repository}.
...
All the harmonics share a sharp boundary around $\delta \approx 0.34$, $V/t \approx 2.5$, visible as a vertical segment in the three heatmaps on the left column of Fig.~\ref{fig:sc-data-surface-wS-B256}. The isotropic harmonics suddenly change from zero to large values moving from left to right, while the $d$-wave part does the opposite. With our coarse-grained simulations, we were not able to verify if this behaviour underlies some deeper structure.

Summarising all results, we can distinguish four phases: the normal state and three SC regions (pure $d$-wave, pure $s \oplus s^*$-wave, mixed). In Fig.~\ref{fig:sc-mix}, we show the phase diagram in the plane $(\delta,V)$ for $U/t=0$ (left), $U/t=4$ (centre) and $U/t=12$ (right). The boundaries were computed by setting a threshold $\varepsilon=10^{-2}$, with the following rule:
\begin{center}
...
\begin{figure}
	...
	\caption{Depiction of the normal state region and the three singlet SC regions, at $U/t=0,4,12$ from left to right. Lighter blue indicates the pure $d$-wave SC phase, blue the isotropic $s \oplus s^*$ phase, deeper blue the mixed region. In text is described the convention we used to identify the boundaries.}
	\label{fig:sc-mix}
\end{figure}
...
\begin{enumerate}
	\item turning on the $s$-wave gap harmonic at finite doping (see Fig.~\ref{fig:sc-data-ws-AB256-compared}), whereas for $U=0$ the local isotropic component of the gap $\tilde{\Delta}^{(s)}$ is identically zero;
	\item expanding the $d$-wave region and restricting the isotropic and mixed region. This is consistent with the observation that the local repulsion $U$ enhances Mott effects, which in turn produce $d$-wave superconductivity \cite{keimer2015quantum}.
\end{enumerate}
...
As we discussed in Sec.~\ref{subsec:normal-d-wave-hopping}, and at the end of Sec.~\ref{sec:sc-hf-hfps}, nematicity is established whenever $\tilde{t}^{(d)}$ becomes non-zero. If the renormalised hopping parameter has a non-zero $d$-wave component, this means that bare hopping is effectively boosted in one direction and damped in the other. Indeed, from Eqs.~\eqref{eq:sc-tx}, \eqref{eq:sc-ty}, we know that
\[
...
\begin{figure}
	...
	\caption{
%		Dependency of the $d$-wave renormalised hopping $\tilde{t}^{(d)}$ varying the doping $\delta$ and the local repulsion $U$. In the top panel we compare three doping levels $\delta=0.0,~0.2,~0.4$ varying $0 \le U \le 8$. In the bottom panel we compare three repulsion values $U=0.0,~5.0,~10.0$ varying $0 \le \delta \le 0.45$.
		Dependency of the $d$-wave component of the renormalised hopping $\tilde{t}^{(d)}$ on $V$ at fixed $U$ and $\delta$ (top panel, compared figures) and on $\delta$ at fixed $V$ and $U$ (bottom panel, compared figures).
	}
	\label{fig:sc-data-td-AB256-compared}
\end{figure}

For the normal and AF phases we were able to prove that $u^{(d)}=0$ for any choice of model parameters. For the SC phase, instead, $u^{(d)}$ converges to non-zero values. In Fig.~\ref{fig:sc-data-td-AB256-compared}, we show the quantity
\[
	\tilde{t}^{(d)} = -\frac{V u^{(d)}}{2} \, ,
\]
as a function of $U$, $V$ and $\delta$. In the top panel (Fig.~\ref{subfig:sc-data-td-A256-compared}), we see that a critical value of $V$ exists over which nematicity is established. Moreover, the $\delta=0.0$ case (left panel) is independent of $U$. In all cases, the convergence values for $\tilde{t}^{(d)}$ are two order of magnitudes smaller than $t=1.0$, meaning that nematicity is a sharp but small effect. Note that at $\delta=0.2$ (central panel) $\tilde{t}^{(d)}\ge0$, while at $\delta=0.4$ (right panel) $\tilde{t}^{(d)}\le0$. Increasing the doping we change the sign of $\tilde{t}^{(d)}$.
...
\begin{figure}
	...
	\caption{
%		Dependency of the $s^*$-wave renormalised hopping $t^{(s^*)}$ varying the doping $\delta$ and the local repulsion $U$. In the top panel we compare three doping levels $\delta=0.0,~0.2,~0.4$ varying $0 \le U \le 8$. In the bottom panel we compare three repulsion values $U=0.0,~5.0,~10.0$ varying $0 \le \delta \le 0.45$.
		Dependency of the $s^*$-wave component of the renormalised hopping $\tilde{t}^{(s^*)}$ on $V$ at fixed $U$ and $\delta$ (top panel, compared figures) and on $\delta$ at fixed $V$ and $U$ (bottom panel, compared figures).
	}
...
\]
In Fig.~\ref{fig:sc-data-tS-AB256-compared}, we plot $\tilde{t}^{(s^*)}$ as a function of $U$, $V$ and $\delta$. In the top panel (Fig.~\ref{subfig:sc-data-tS-A256-compared}), we show the dependence of $\tilde{t}^{(s^*)}$ on $V$. The three curves, obtained at three doping levels, are approximately independent of $U$. The common feature is that $\tilde{t}^{(s^*)}$ decreases linearly with $V$ up to some critical value, above which it converges to a constant value which increases as a function of the doping. As was for the AF and normal phases, electronic doping suppresses the isotropic bands shrinking effect. At half-filling (left figure) we see that bands shrinking is most effective, reducing the hopping parameter to less than half its $V=0$ value. In the bottom panel (Fig.~\ref{subfig:sc-data-tS-B256-compared}) we show $\tilde{t}^{(s^*)}$ as a function of $\delta$, varying $V$ and $U$. As we already noted, the dependence on $U$ is very weak and the three figures are a lot alike. As we already mentioned, $\tilde{t}^{(s^*)}$ increases with doping.
...
In Fig.~\ref{fig:sc-tS-vs-n}, we compare $\tilde{t}^{(s^*)}$ for the normal phase (blue surface) and the SC phase (green surface). The two surfaces share a general feature: $\tilde{t}^{(s^*)}$ is approximately unitary at low $V/t$ and almost unitary doping, and decreases significantly in the low-$\delta$ and high-$V/t$ limit. In this region, for the normal phase we observe the Mott-localisation plateau of Fig.~\ref{fig:normal-tS-data}, discussed in Sec.~\ref{subsec:normal-mott-numeric}, where $\tilde{t}^{(s^*)} \approx 0$. In the SC phase, looking at the same region, $\tilde{t}^{(s^*)}$ is minimised, but, in contrast to the normal phase, we do not observe the complete Mott localisation effect.

Recalling the plots of Fig.~\ref{fig:sc-mix}, the low-$\delta$ and high-$V/t$ limit (the corner where $\tilde{t}^{(s^*)}$ is minimised) falls deep within the mixed region. Recalling also Fig.~\ref{fig:sc-data-surface-wS-B256}, in this corner the $d$-wave and $s^*$-wave harmonics of the gap function are large and comparable. Finally, we know from Fig.~\ref{fig:sc-td} (central panel) that here nematic effects are small, but significantly non-zero. In particular, considering that 
\[
...
In Fig.~\ref{fig:sc-data-fs-B256-compared} we show $f$ and $s$ as functions of $V$, varying $\delta$ and comparing for three values of $U$. On both rows, moving from left to right we do not observe significant differences: $f$ and $s$ depend weakly on $U$. We already observed the presence of a maximum of the free energy at ratios $V/t$ of order unity. In Fig.~\ref{subfig:sc-data-f-B256-compared} we see this maximum around $V/t \lesssim 4$. Comparing with the data of Fig.~\ref{fig:sc-data-fs-B128}, we can imagine that this maximum could be located at at the boundary between $d$-wave and mixed regions.
...
\]
and observe that it is greater or equal to zero over the entire explored region. The energy difference is null only at $V/t \lesssim 2$, where the SC gap is null. We plot, superimposed, the boundaries of the normal region extracted from Fig.~\ref{fig:sc-mix}. In the rest of the $(\delta,V)$ plane the SC phase is energetically favoured with respect to the normal phase. 

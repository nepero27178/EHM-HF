...
In this chapter we study the translationally invariant superconducting (SC) phase. We discuss the formation of Cooper pairs in given spin-pairing channels, and derive the self-consistent solution in both the singlet and triplet channels. The solution will exhibit a number of effects: bare bands shrinking, insurgence of mixed-symmetry superconductivity and nematic effects.

This chapter is organised as follows. In Sec.~\ref{sec:sc-phase-theory} we present the translationally invariant superconducting phase and its symmetries. In Secs.~\ref{sec:sc-hm}, \ref{sec:sc-ehm}, \ref{sec:sc-hf-hamiltonian} we derive the approximated HF hamiltonian for the SC phase, starting from the conventional Hubbard model and then adding the non-local attraction $\hat H_V$. In Sec.~\ref{sec:sc-sceq-analysis} we derive a constraints on the HFPs by analysing their self-consistency equations. In Sec.~\ref{sec:sc-numeric} we present the result of numeric simulations.
...
\end{aligned}
\]
Translational invariance is unbroken. Then electronic density needs to be uniform across all sites, similarly to the normal phase discussed in Chap.~\ref{chap:normal}. Non-uniform density distributions are inconsistent with translational invariance in both directions. In particular, Eqs.~\eqref{eq:normal-<cc>} and \eqref{eq:normal-<n_i>} hold identically,
\begin{align}
...
\section{The SC phase in the HM: application of HF methods}\label{sec:sc-hm}
...
\begin{tabproof}[Cooper pairs have zero net momentum]
	...
	\]
	where we used the convention of Eq.~\eqref{eq:fourier-convention}. Let now $\mathbf{r} \equiv \mathbf{r}_1$ and $\Delta \mathbf{r} \equiv \mathbf{r}_1 - \mathbf{r}_2$. Translational invariance implies that
	\[
...
\begin{equation}\label{eq:sc-HU-gap-phi}
	...
	\right] - E_{\mathrm{C}/U}^{(\mathrm{SC})}
\end{equation}
This last decomposition resembles Eq.~\eqref{eq:af-HU-hartree-approx}, obtained for the AF phase in the Hartree channel.
...
Let $\Delta_\mathbf{k} = \Delta^{(s)}$. The HM hamiltonian is given by
\[
	\hat H_t + \hat H_U
\]
and its HF approximation in the SC phase can be written as
...
\end{equation}
The definition of the shifted bare bands $\xi_\mathbf{k}$ contains the renormalised chemical potential, not $\mu$. We defined the Nambu spinors
\begin{equation}\label{eq:sc-Phi-def}
	...
	\end{bmatrix}
\end{equation}
In Eq.~\eqref{eq:sc-HM-hk}, we also defined the ``anticommutation energy'' $E_\mathrm{a}^{(\mathrm{SC})}$,
...
\begin{tabproof}[Proof of Eq.~\eqref{eq:sc-HM-hk}]
	...
	Summing Eq.~\eqref{eq:Ht-Fourier} and Eq.~\eqref{eq:sc-HU-gap-phi}, we obtain the approximated hamiltonian
	\begin{multline}
	...
	\end{multline}
	Due to inversion symmetry, it holds $\xi_\mathbf{k} = \xi_{-\mathbf{k}}$. Using Fermi anticommutation rules, the kinetic operator can be written as:
	\[
	\begin{aligned}
	...
	\end{aligned}
	\]
	Inserting the last line in Eq.~\eqref{eq:sc-hm-mft-h}, we obtain
	\begin{multline*}
...
Te unique HFP that we obtained for the SC phase is $w^{(s)}$. It obeys the self-consistency equation
\begin{equation}\label{eq:sc-hm-ws-self-consistency-2}
	\frac{1}{2 L_x L_y} \sum_{\mathbf{k}\in\mathrm{BZ}} \frac{\Delta_\mathbf{k}}{E_\mathbf{k}} \mathrm{Th}_\mathbf{k}^{(-)}[\beta]
\end{equation}

\begin{tabproof}[Self-consistency equation for $w^{(s)}$]

	Let
	\[
		\tau^\pm \equiv \frac{\tau^x \pm i \tau^y}{2}
	\]
	From Eq.~\eqref{eq:sc-hm-ws-self-consistency}, we obtain
	\[
	\begin{aligned}
...
\begin{equation}\label{eq:sc-hm-Uc}
	...
	\right]^{-1}
\end{equation}
We call it ``critical'' because its value gives a sort of boundary for convergence. Indeed, {\color{tabred}\textbf{as we show in App.~\ref{app:sc-g-opt}}}, for the SC phase in conventional HM, we are able to optimise parametrically the mixing parameter $g$ in order to ensure convergence, based on the ratio $U/U_\mathrm{c}[\beta]$. Let us show that superconductivity is not possible in the conventional HM with $U>0$.
...
\section{Expansion to the EHM}\label{sec:sc-ehm}
...
\end{align}
where we used Eq.~\eqref{eq:normal-HV-fock-Ec}, which is formally identical in the SC phase with the only difference that EVs are now computed on the SC ground-state. To get to this point, we used the fact that:
\[
...
Let us prove the above equation. We proceed by proving first $u_{\mathbf{k}\uparrow} = 	u_{-\mathbf{k}\downarrow}$, and that both transform accordingly to Eq.~\eqref{eq:sc-hm-ugamma-self-consistency}
...
\begin{tabproof}[Self-consistency equations for $u^{(\gamma)}$]
	...
	\end{align}
	having we used Eq.~\eqref{eq:sc-~Th} and sequent. Inserting this result in Eq.~\eqref{eq:sc-hm-ugamma-harmonics}, we get Eq.~\eqref{eq:sc-hm-ugamma-self-consistency} in this spin sector. For $\sigma=\downarrow$,
	\begin{align}
	...
	\]
	which is coherent with our requirement of a uniform-density, spin uniform and inversion symmetric phase. This proves Eq.~\eqref{eq:sc-hm-ugamma-self-consistency} for both spin sectors.
\end{tabproof}
%
...
\begin{tabproof}[Bogoliubov bands $\pm \tilde{E}_\mathbf{k}$ give equal entropy densities.]
	...
	\]
	Inserting the latter for $\epsilon=\tilde{E}_\mathbf{k}$ in the expression for $s_-^{(\mathrm{SC})}$, we get
	\[
		s_-^{(\mathrm{SC})} \equiv -\frac{k_B}{L_x L_y} \sum_{\mathbf{k}\in\mathrm{BZ}} \Big[
			f(
			\tilde{E}_\mathbf{k}; \beta,0)
		...
		\Big]
	\]
	hence $s_-^{(\mathrm{SC})}=s_+^{(\mathrm{SC})}$.	
\end{tabproof}
%
...
\end{equation}
Note that, as the gap closes and $\tilde{E}_\mathbf{k} \to \tilde{\xi}_\mathbf{k}$, the above expression reduces to the entropy density for the normal phase reported in Eq.~\eqref{eq:normal-s-variant}.
...
\begin{tabproof}[Free energy density for the SC phase]
...
\section{Analysis of the solution} \label{sec:sc-sceq-analysis}

In this section we derive one constraint on the solution for the SC phase, which ``regulates'' the competition among various symmetries of the superconducting gap. We specify that, differently from AF and normal phases, for the singlet SC phase we did not implement any theoretical constraint on the HFPs $u^{(\gamma)}$, $v^{(\gamma)}$. As we will discuss, all HFPs are significantly non-zero in this phase, at least in some portion of parametric space.

\paragraph{Energetic constraints.}

We derived the expression of Eq.~\eqref{eq:sc-fMFT}, which contains in its second row the HFPs $u^{(\gamma)}$ and $w^{(\gamma)}$. In this section, similarly to what was done in Sec.~\ref{sec:af-sceq-analysis} for the AF phase, we give a proof of the following equation:
...
This proof ends the theoretical discussion of the SC phase. Unfortunately, we are not able to provide any more constraints on the HFPs, differently from AF and normal phase. Fortunately, as we will show with the analysis of numerics in next section, there is no particular need for additional constraints, at least for the $w^{(\gamma)}$ HFPs. We would have liked to have a proof that $0 \le u^{(s^*)} \le 2t/V$ also for the SC phase, but we do not have any. 

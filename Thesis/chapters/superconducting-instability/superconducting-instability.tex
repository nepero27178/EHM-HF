...
In this chapter, we study the translationally invariant superconducting (SC) phase. We derive the self-consistent HF hamiltonian in both the singlet and triplet channels, and simulate only in the singlet channel. We will observe various effects: bare bands shrinking, insurgence of mixed-symmetry superconductivity and nematicity.

This chapter is organised as follows. In Sec.~\ref{sec:sc-phase-theory} we present the translationally invariant superconducting phase and its symmetries. In Secs.~\ref{sec:sc-hm}, \ref{sec:sc-ehm}, \ref{sec:sc-hf-hamiltonian} we derive the approximated HF hamiltonian for the SC phase, starting from the conventional Hubbard model and then adding the non-local attraction $\hat H_V$. In Sec.~\ref{sec:sc-sceq-analysis} we derive a constraint on the HFPs by analysing their self-consistency equations. In Sec.~\ref{sec:sc-numeric} we present the result of numeric simulations.

{\color{tabred}\paragraph{Disclaimer.}
	
Sec.~\ref{sec:sc-ehm}, where we carry out all technical computations to derive the HF approximation to the EHM, is \textit{very} detailed and cumbersome. It can be skipped, jumping directly to Sec.~\ref{sec:sc-hf-hamiltonian}, where we summarise all results.}

\section{The SC phase and its symmetries}\label{sec:sc-phase-theory}

In Sec.~\ref{sec:ehm-sym-break} we anticipated that the translationally invariant SC phase is described by the two SSBs
\[
\begin{aligned}
	\mathrm{U}^z(1) &\SSB 0 \, , \\
...
\end{aligned}
\]
Translational invariance is unbroken. Therefore, Eqs.~\eqref{eq:normal-<cc>} and \eqref{eq:normal-<n_i>}, derived in the context of the normal phase, hold equivalently,
\begin{align}
...
%	(\text{with $i,j$ NNs})
%\]
%are, in general, non-zero. The ground state plus two electrons can have a finite overlap with itself. This corresponds to the phenomenon of Cooper pairing \cite{coleman2015introduction}, where pairs of electrons are bound together.
%
%Cooper pairs are composite quantum objects, with spatial and spin structure. Being couples of electrons, they must be antisymmetric under internal exchange. Moreover, as any composite object, a Cooper pair has a total spin whose algebra follows the composition rules of basic Quantum Mechanics,
%\[
...
\]
For the conventional HM, we know from Sec.~\ref{subsec:sc-hm-diagonalisation} that $w^{(s)}=0$. For the EHM, it can assume non-zero values, as long as Eq.~\eqref{eq:sc-wgamma-circle} is satisfied. In other words, $w^{(s)}$ cannot be non-zero without at least one of the other two singlet harmonics -- namely, $s^*$-wave and $d$-wave -- being non-zero.
...
This proof ends the theoretical discussion of the SC phase. Differently from the other phases, we are not able to provide any more constraints on the HFPs.

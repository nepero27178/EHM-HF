...
\end{enumerate}
This concept is summarised in Tab.~\ref{tab:sc-pairings-symmetry-structures}. We can choose to restrict to just one of these pairing channels. This provides specific symmetry selection rules on the SC gap function harmonics. In this thesis, we restrict to the singlet channel.
...
\begin{enumerate}
	\item turning on the $s$-wave gap harmonic at finite doping (see Fig.~\ref{fig:sc-data-ws-AB256-compared});
...
\]
In Fig.~\ref{fig:sc-data-tS-AB256-compared}, we plot $\tilde{t}^{(s^*)}$ as a function of $U$, $V$ and $\delta$. In the top panel (Fig.~\ref{subfig:sc-data-tS-A256-compared}), we show the dependence of $t^{(s^*)}$ on $V$. The three curves, obtained at three doping levels, are approximately independent of $U$. The common feature is that $\tilde{t}^{(s^*)}$ decreases linearly with $V$ up to some critical value, above which it converges to a constant value which increases as a function of the doping. As was for the AF and normal phases, electronic doping suppresses the isotropic bands shrinking effect. At half-filling (left figure) we see that bands shrinking is most effective, reducing the hopping parameter to less than half its $V=0$ value. In the bottom panel (Fig.~\ref{subfig:sc-data-tS-B256-compared}) we show $t^{(s^*)}$ as a function of $\delta$, varying $V$ and $U$. As we already noted, the dependence on $U$ is very weak and the three figures are a lot alike. As we already mentioned, $t^{(s^*)}$ increases with doping.
...
\begin{itemize}
	...
	\item In the SC phase, we open a gap at the chemical potential, not at zero energy (as instead was in the AF phase). Computing entropy as in Sec.~\ref{subsec:sc-s}, we apply the Fermi-Dirac distribution on the two bands $\pm\tilde{E}_\mathbf{k}$. As the gap opens, the lower one fills (occupation $\approx 1$), while the upper empties (occupation $\approx 0$). As we lower temperature, the above statement gets exact. Recalling Fig.~\ref{subfig:normal-entropy}, we see that entropy is minimised at occupation equal to $0$ or $1$. Hence, the SC phase is low-entropic.
\end{itemize}

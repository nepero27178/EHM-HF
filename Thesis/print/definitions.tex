% Chapter layout
\setsecnumdepth{subsection}
\maxtocdepth{subsection}

% Tab layout (align equations at different rows)
\newcolumntype{E}{r@{${}={}$}l}
\newcolumntype{T}{r@{${}\to{}$}l}
%\newcolumntype{AND}{r@{${}\land{}$}l}
%\newcolumntype{OR}{r@{${}\lor{}$}l}

% Custom commands
\newcommand{\beq}{\begin{equation}}
\newcommand{\eeq}{\end{equation}}
\newcommand{\sgn}{\mathrm{sgn}}
\newcommand{\red}[1]{\color{tabred}{#1}}
\newcommand{\mref}{{\color{red}[Missing]}} % missing ref
\newcommand{\todo}{\noindent{\color{tabred}[To be continued\dots]}}
\newcommand{\Id}{\mathbb{I}}
\newcommand{\hc}{\mathrm{h.c.}}
\newcommand{\Feq}{\stackrel{\mathcal{F}}{=}}
\newcommand{\Weq}{\stackrel{\mathrm{W}}{=}}
\newcommand{\Ieq}{\stackrel{!}{=}}
\newcommand{\HFsimeq}{\stackrel{\mathrm{HF}}{\simeq}}
\newcommand{\SSB}{\stackrel{\mathrm{SSB}}{\to}}

\newcommand{\blankpage}{
	\newpage
	\thispagestyle{empty}
	\mbox{}
	\newpage
}

% Colors and shadings
\definecolor{tabred}{RGB}{214, 39, 40}
\definecolor{tabblue}{HTML}{1f77b4}
\definecolor{tabgreen}{HTML}{2ca02c}
\tikzfading[
	name=fade out,
	inner color=transparent!0,
	outer color=transparent!100
]

% Custom colormap
\pgfplotsset{
	colormap={tab}{
		color=(tabred)
		color=(lightgray)
		color=(tabblue)
	}
}

% Custom 3d tilted plane canvas definition
\tikzoption{canvas is plane}[]{\@setOxy#1}
\def\@setOxy O(#1,#2,#3)x(#4,#5,#6)y(#7,#8,#9)%
{\def\tikz@plane@origin{\pgfpointxyz{#1}{#2}{#3}}%
	\def\tikz@plane@x{\pgfpointxyz{#4}{#5}{#6}}%
	\def\tikz@plane@y{\pgfpointxyz{#7}{#8}{#9}}%
	\tikz@canvas@is@plane
}

%\usepackage[usenames,dvipsnames]{color} % more flexible names for syntax highlighting colors

% Code
\lstset{
    basicstyle=\ttfamily, 
    numbers=left, 
    numberstyle=\small\ttfamily\color{tabgreen},
    stepnumber=1,              
    numbersep=10pt, 
    numberfirstline=true, 
    numberblanklines=true, 
    tabsize=4,
    lineskip=0pt,
    aboveskip=10pt,
    belowskip=10pt,
    extendedchars=true,
    breaklines=true,
    %backgroundcolor=\color{ForestGreen!10},
    keywordstyle=\color{tabblue}\bfseries,
    identifierstyle=, % using emph or index keywords
    commentstyle=\color{tabgreen},
    stringstyle=\color{tabred},
    showstringspaces=false,
    showtabs=false,
    upquote=false,
    texcl=true % interpet comments as LaTeX
}

\lstdefinelanguage{julia}
{
  keywordsprefix=\@,
  morekeywords={
    exit,whos,edit,load,is,isa,isequal,typeof,tuple,ntuple,uid,hash,finalizer,convert,promote,
    subtype,typemin,typemax,realmin,realmax,sizeof,eps,promote_type,method_exists,applicable,
    invoke,dlopen,dlsym,system,error,throw,assert,new,Inf,Nan,pi,im,begin,while,for,in,return,
    break,continue,macro,quote,let,if,elseif,else,try,catch,end,bitstype,ccall,do,using,module,
    import,export,importall,baremodule,immutable,local,global,const,Bool,Int,Int8,Int16,Int32,
    Int64,Uint,Uint8,Uint16,Uint32,Uint64,Float32,Float64,Complex64,Complex128,Any,Nothing,None,MPO,MPS,String,
    function,type,typealias,abstract
  },
  sensitive=true,
  morecomment=[l]{\#},
  morestring=[b]',
  morestring=[b]" 
}

% Captions

\newcommand*{\CaptionVLineWidth}{0.5pt}
\newcommand*{\CaptionVLineSep}{.5em}

\newcommand*{\CaptionVLine}{%
	\noindent
	\kern\dimexpr-\CaptionVLineSep-\CaptionVLineWidth\relax
	\textcolor{gray}{\vline width\CaptionVLineWidth}%
	\kern\CaptionVLineSep\relax
}

\DeclareCaptionFormat{vLine}[%
#1#2\CaptionVLine#3\par % Single line captions
]{% Multi-line captions
	\caption@ifin@list\caption@lsepcrlist\caption@lsepname{%
		\caption@Error{%
			The option `labelsep=\caption@lsepname' does not work\MessageBreak
			with `format=hang'}
	}{%
		\sbox0{%
			\parbox[t]{\linewidth}{%
				\@hangfrom{#1#2}%
				\advance\caption@parindent\hangindent
				\advance\caption@hangindent\hangindent
				\xdef\CaptionHangIndent{\the\hangindent}%
				\caption@@par#3\par
			}%
		}%
		\noindent
		\kern\CaptionHangIndent\relax
		\CaptionVLine
		\kern-\CaptionHangIndent\relax
		\usebox0%
	}%
}

% Customize captions

\captionsetup{
	labelsep=quad,
	labelfont={color=gray},
	format=vLine
}
\captionsetup[sub]{
	labelsep=quad,
}


% Proof environment

\newcounter{proof}
\counterwithin{proof}{chapter}
\newenvironment{tabproof}[1][]
{
\refstepcounter{proof}
\vspace{1em}
\noindent\normalfont   
\textbf{Proof~\theproof} | #1.
\nopagebreak
\begin{mdframed}[
	linewidth=0.5pt,
	linecolor=gray,
	leftmargin=1em,
	bottomline=false,
	topline=false,
	rightline=false,
	innerrightmargin=0pt,
	innertopmargin=0pt,
	innerbottommargin=0pt,
	innerleftmargin=1em,
	skipabove=1em,
	skipbelow=1em
	]
}
{\end{mdframed}}
%\AtEndEnvironment{tabproof}{\hfill$\square$}%

\newgeometry{
	left=2.5cm,
	right=2.5cm}
	\thispagestyle{plain}
	\begin{table}
		\centering
		\begin{tabular}{r l}
			\multicolumn{2}{c}{\textbf{List of symbols and abbreviations}} \\
			\midrule
			$\HFsimeq$ & Hartree-Fock approximation \\
			$\Ieq$ & Imposition of a mathematical condition \\
			$\Feq$ & Computation of a Fourier transform \\
			\midrule
			AF & Antiferromagnetic \\
			BCS & Bardeen-Cooper-Schrieffer (theory) \\
			BZ & Brillouin zone \\
			$\beta$ & Inverse temperature \\
			DoF & Degree of freedom \\
			DoS & Density of states \\
			EHM & Extended Hubbard model \\
			EV(s) & Expectation value(s) \\
			FS & Fermi surface \\
			HF & Hartree-Fock \\
			HFP(s) & Hartree-Fock parameter(s) \\
			HM & Hubbard model \\
			$\mathcal{L}$ & The set of vertices of the square lattice \\
			$L_\ell$ & Number of lattice sites in direction $\ell$ \\
			iHF & Iterative Hartree-Fock \\
			MBZ & Magnetic Brillouin zone \\
			MFT & Mean Field Theory \\
			NBZ & Nematic Brillouin zone \\
			NN(s) & Nearest neighbor(s) \\
			PHS & Particle-hole symmetry \\
			RMP(s) & Renormalised model parameter(s) \\
			SC & Superconducting \\
			SDW & Spin-density wave \\
			SSB & Spontaneous symmetry breaking \\
			$T_\mathrm{c}$ & Critical temperature \\
			\midrule
		\end{tabular}
	\end{table}
\restoregeometry 

...
\begin{table}
	...
	\begin{tabular}{c l}
		...
		\midrule
	\end{tabular}
	\caption{Model symmetries and relative group operations. Note that $\mathrm{U}^\mathrm{c}(1)$ represents the charge conserving group, while $\mathrm{U}^z(1)$ represents the subgroup obtained by breaking the $\mathrm{SU}(2)$ symmetry with a vector order parameter and preserving its amplitude.}
...
\subsection{Particle-hole symmetry at half-filling}

\todo

\subsection{The role of spin}
\todo
...
\subsection{MFT general procedure and self-consistency}

The Mean Field Theory approach to a fermionic quartic is given by the following procedure: given the hamiltonian made of the quadratic operators $\hat A$ and $\hat B$
\[
\hat H = \hat A \hat B
\]
...
The pure Hubbard model, $\hat H_t + \hat H_U$, offers a simple framework for MFT analysis of the AF phase: this is described in detail in App.~\ref{appendix:mean-field-hubbard}. The key passage is there given by the MFT approximation
...
In terms of the discussion of the above paragraph, we have $\hat A \equiv \hat n_{i\uparrow}$ and $\hat B \simeq \hat n_{i\downarrow}$. However the ``full'' operator is given by
\[
...
\begin{itemize}
	...
	\item The Hartree term can break translational invariance (based on our Ansatz on the expectation value), because the mean-field to interact with is given by the local density. $\mathrm{SU}(2)$ symmetry is also broken, because we do not have spin DoF perfect degeneracy anymore, but $\mathrm{U}^z(1)$ symmetry still holds.
\end{itemize}
As a general rule of thumb, throughout this text we will discard the o.s. Fock term eveywhere. For the normal state as well as for the AF instability only the Hartree term is to be accounted; in superconducting instability also the Cooper term contributes. Note that for superconducting instabilities, due to superexchange mechanism (as explained in App.~\ref{appendix:mean-field-hubbard}) the o.s. term account for singlet pairing as well as zero-spin triplet pairing. Which channel is preferred, is a matter of thermodynamic advantage.
...
As a final remark, notice that the superconducting instabilities of the s.s. terms account only for triplet pairing. The only possible superconducting ordering established by the means of these terms is odd in real space. Then $s$-wave and $d$-wave superconductivity cannot establish in this channel; $p_\ell$-wave superconductivity, instead, can.
...
\[
	\hat c_{j\sigma} \Feq \frac{1}{\sqrt{L_xL_y}} \sum_{\mathbf{k} \in \mathrm{BZ}} e^{-i \mathbf{k} \cdot \mathbf{x}_j} \hat c_{\mathbf{k}\sigma}
\]
Then:
\[
\begin{aligned}
...
\begin{align}
	...
	&\Feq - \frac{V}{L_x L_y} \sum_{\sigma\sigma'} \sum_{\ell = x,y} \sum_{\mathbf{K}, \mathbf{k}, \mathbf{k}'} \cos\left(
...
\end{equation}
When contracting these fermionic operators, identical considerations as for the non-local counterpart \eqref{eq:HV-fourier} hold. Note the similarity of this Fourier transform with the o.s. sector of the non-local attraction, given in Eq.~\eqref{eq:HV-fourier}.
...
\section{The role of $2$D symmetry channels}

Looking at $\hat H$, the main difference between its two-body interactions $\hat H_U$ and $\hat H_V$ of course relies in locality (first) and non-locality (second). As any two-body interaction, $\hat H_V$ can be decomposed in harmonics. At a first look one may think that, being the potential symmetric by $\pi/2$ rotations on the lattice, the resulting ground state should exhibit only properties with identical symmetry. However this is not the case: said symmetry is a property of the hamiltonian operator on the square lattice, however the resulting ground state is a composite object living in a many-body Fock space, \todo
...
\]
where $g^{(\gamma)}$ are the $g_{ij}$ symmetries-decomposition coefficients while $\varphi_{ij}^{(\gamma)}$ are the form factors listed in Tab.~\ref{tab:x-wave-real-factors} and plotted in Fig.~\ref{fig:x-wave-reciprocal-factors}, a simple orthonormal rearrangement of the harmonics basis.
...
\subsection{Symmetry channels structure factors decomposition}\label{subsec:symmetry-channels-decomposition}

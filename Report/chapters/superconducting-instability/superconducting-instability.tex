...
For the Hartree channel, being the density uniform across sites, we can re-use the entirety of Sec.~\ref{subsec:normal-hartree-analytic} for the normal phase. Substituting Eq.~\eqref{eq:sc-density-ansatz}, we get the approximation of Eq.~\eqref{eq:normal-HU-hartree-approx},
\begin{equation}\label{eq:sc-HU-approx}
	\hat H_U \HFsimeq nU \times \hat N - E_{\mathrm{H}/U}^{(\mathrm{SC})} + \text{(Cooper channel)}
\end{equation}
thus chemical potential renormalisation $\tilde{\mu} \to \mu - nU$ is present also in the SC phase. The contraction energy is the same of Eq.~\eqref{eq:normal-Ec-hU}
...
\end{multline*}
where we used Eq.~\eqref{eq:mft-HV}. This discussion is based on that for the normal phase in Sec.~\ref{subsubsec:normal-hartree-mu}, and leads to an identical result: the non-local Hartree channel renormalises the chemical potential.

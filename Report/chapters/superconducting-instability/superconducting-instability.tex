...
\[
	\lim_{\delta \to 0} \lim_{V/t \gg 1} \frac{t^{(s^*)}}{t} \approx 0.4
	\qquad
	\lim_{\delta \to 0} \lim_{V/t \gg 1} \frac{t^{(d)}}{t} \approx 0.06
\]
...
For the SC phase, free energy density is given by Eq.~\eqref{eq:sc-fMFT}. Entropy density is obtained by Eq.~\eqref{eq:sc-density-entropy} using that $s=2s^{(+)}$, as we argued in the sequent lines.

\begin{figure}
	\centering
	\subfloat[Dependency of $f$ on $V$ at various $U$s, compared at three values of $\delta$.]{
...
In Fig.~\ref{fig:sc-data-fs-B256-compared} we show the data for $f$ and $s$ varying 

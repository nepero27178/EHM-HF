...
\begin{figure}
	...
	\caption{Depiction of the normal state region and the three singlet superconducting regions. Lighter blue indicates the pure $d$-wave SC phase, blue the isotropic $s \oplus s^*$ phase, deeper blue the mixed region. In text is described the convention we used to identify the boundaries.}
	\label{fig:sc-mix}
\end{figure}

Comparing the two panels of Fig.~\ref{fig:sc-mix}, we see that increasing the value of the local repulsion $U$ the intersection point of the four regions moves. In particular, the $d$-wave region expands to the right (larger $\delta$s), and the critical value for $V$ for the transition from normal state to $s \oplus s^*$ superconductor moves up (larger $V$s). The mixing region is pretty much unchanged, going from $V/t \approx 3$ at low doping to higher values at almost unitary filling.

In next section we show that mixing is the root cause of nematic symmetry breaking,
\[
	\mathrm{C}_4 \SSB \mathrm{C}_2
\]
...
\begin{itemize}
	\item there exists a node, some special doping $\delta_0$ at which $\tilde{t}^{(d)}=0$ and the resulting superconductor is not nematic;
	\item at higher doping the direction of nematic symmetry breaking is exchanged.
\end{itemize}
At $=10.0$ (right panel) we see that $t^{(d)}\ge0$ at all $V$s. Sign change, if present, must occur at higher values. In the end, increasing the value of the local repulsion $U$ we do not prevent nematicity, rather we suppress this secondary nematic effect, originated from the change of sign of $\tilde{t}^{(d)}$.
...
Nematic effects are directly related to symmetry mixing in the superconducting order parameter. Indeed, consider Fig.~\ref{fig:sc-td}. There, we plot the convergence values for $\tilde{t}^{(d)}$ in the $(\delta,V)$ plane. The superimposed white lines are the mixing boundaries, delimiting the deeper blue regions of Fig.~\ref{fig:sc-mix}. The anisotropic hopping $\tilde{t}^{(d)}$ is non-zero in the mixed region. \todo

...
Breaking $\mathrm{U}^z(1)$, we allow EVs like $\langle \hat c^\dagger \hat c^\dagger \rangle$ to be non-zero. This leads to the formation of Cooper pairs. These are composite quantum objects with a specific spin-spatial structure. Consider the generic Cooper fluctuation in o.s. sector,
\[
	\big\langle
...
\]
The two pairing channels are, at this level: the singlet channel (associated to total spin $0$) and the triplet channel (associated to total spin $1$). Now, when applying HF methods, we can impose to the approximated HF ground-state a particular spatial structure -- say, inversion symmetry or antisymmetry. This reflects in the requirement that Cooper pairs show the same spatial structure.
...
\subsection{Local Hartree channel in the SC phase}\label{subsec:sc-l-hartree}
...
\end{equation}
thus chemical potential renormalisation $\tilde{\mu} \to \mu - nU$ is present also in the SC phase.

The contraction energy is the same of Eq.~\eqref{eq:normal-hartree-U-contraction-energy}
...
\subsection{Local Cooper channel in the SC phase}\label{subsec:sc-l-cooper}
...
\end{align}
where we used Eqns.~\eqref{eq:sc-hm-ws-self-consistency-2}, \eqref{eq:sc-hm-n-BZ-Th}. From Tabs.~\ref{tab:HU-sel-rules} and \ref{tab:HV-sel-rules} we know the selection rule for each channel. We break down each.
...
\end{multline*}
where we used Eq.~\eqref{eq:mft-HV}. This discussion is based on that for the normal phase in Sec.~\ref{sec:normal-hartree-mu}, and leads to an identical result: the non-local Hartree channel renormalises the chemical potential.
...
\[
	\hat H_U \HFsimeq nU \times \hat N - E_{\mathrm{H}/U}^{(\mathrm{SC})}
\]
gives the chemical potential full renormalisation, identical to Eq.~\eqref{eq:normal-mu-shift},
...
\section{Fock effects: bare bands renormalisation}\label{sec:sc-fock}
...
\begin{equation}\label{sc-hm-ugamma-harmonics}
...
\begin{tabproof}[Self-consistency equations for $u^{(\gamma)}$]
	...
	\end{align}
	having we used Eq.~\eqref{eq:sc-~Th} and sequent. Inserting this result in Eq.~\eqref{sc-hm-ugamma-harmonics}, we get Eq.~\eqref{sc-hm-ugamma-self-consistency} in this spin sector. For $\sigma=\downarrow$,
	\begin{align}
...
Before proving the above statement, we highlight the similarity of these equations with those for $u^{(\gamma)}$, expressed in Eq.~\eqref{sc-hm-ugamma-self-consistency}. Noting that the harmonic factors $\varphi_\mathbf{k}^{(\gamma)}$ are integrated to zero over the BZ, we could obtain one set of equations from the other by exchanging
\[
	\sin 2 \tilde{\theta}_\mathbf{k} e^{i 2\tilde{\zeta}_\mathbf{k}} \leftrightarrow -\cos 2 \tilde{\theta}_\mathbf{k}
\]
Let us now give a proof to Eq.~\eqref{eq:sc-ehm-wgamma-self-consistency}.
...
\begin{tabproof}[Self-consistency equations for $w^{(\gamma)}$]
	...
	\end{aligned}
	\]
	Finally, from Eq.~\eqref{eq:sc-Th-=tanh}, we can substitute the thermal factor with the hyperbolic tangent. This gives Eq.~\eqref{eq:sc-ehm-wgamma-self-consistency}.
	The derivation above holds for all symmetries, (including $s$-wave).
\end{tabproof}
%
This proof concludes this cumbersome algebraic derivation. In next section we summarise the obtained results, give the final form of the HF hamiltonian and discuss the symmetries of the gap function.

\section{The SC ground-state}
\todo

TODO
\begin{itemize}
	\item all from now on in subsections up to numerics
	\item fix red text
\end{itemize}

\subsection{Full MFT hamiltonian and anisotropic pairing}\label{sec:sc-full-hamiltonian}

In this section we take together everything we said so far. Adding $\hat H_V$ to the conventional HM, we have seen, produces renormalisation of the bare bands, the gap function and the chemical potential. By composing Hartree, Fock and Cooper channels we derive the final form of the HF gran-canonical hamiltonian,
\[
...
We anticipated in Tab.~\ref{tab:sc-pairings-symmetry-structures} that Cooper pairing in the singlet channel requires for the ground-state wavefunction to be inversion-symmetric. Triplet pairing, instead, requires inversion anti-symmetry.
...
\[
	...
	\right]
\]
In this case, the gap is purely imaginary. Anyway, we can absorb this phase shift through a gauge transform.

In next section we give an explicit example of how the gap function and the wavefunction properties under spatial inversion are connected, at half filling and zero temperature.
...
\]
For the sake of conciseness, we skip plotting $u^{(s^*)}$ and plot directly $\tilde{t}^{(s^*)}$ in Fig.~\ref{fig:sc-data-tS-AB256-compared}.

In the top panel (Fig.~\ref{subfig:sc-data-tS-A256-compared}), we show the dependence of $t^{(s^*)}$ on $V$. The three curves, obtained at three doping levels, are approximately independent of $U$. The common feature is that $\tilde{t}^{(s^*)}$ decreases somewhat linearly with $V$ up to some critical value, above which it flattens to an horizontal asymptote. Electronic doping changes primarily the height of this asymptote
...
For the SC phase, free energy density is given by Eq.~\eqref{eq:sc-fMFT}. Entropy density is obtained by Eq.~\eqref{eq:sc-density-entropy},
\begin{equation}\label{eq:sc-density-entropy-explicit}
	s \equiv -\frac{2k_B}{L_x L_y} \sum_{\mathbf{k}\in\mathrm{BZ}} \Big[
	\left(
	1- f( \tilde{E}_\mathbf{k}; \beta,0)
	\right) \ln \left(
	1- f( \tilde{E}_\mathbf{k}; \beta,0)
	...
	\Big]
\end{equation}
We used the fact that $s=2s^{(+)}$, as we argued at the beginning of Sec.~\ref{sec:sc-free-energy}.
...
\begin{figure}
	...
	\caption{Heatmaps of the free energy and entropy densities at $U/t=4$. The black dotted lines represent the internal boundaries of the singlet SC phase.}
...
\begin{itemize}
	...
	\item In the SC phase, we open a gap at the chemical potential, not at zero energy (as instead was in the AF phase). In Eq.~\eqref{eq:sc-density-entropy-explicit} we apply the Fermi-Dirac distribution on the two bands $\pm\tilde{E}_\mathbf{k}$: as the gao widens, the lower one fills ($f \approx 1$), while the upper empties ($f \approx 0$). As we lower temperature, the above statement gets exact. Recalling Fig.~\ref{subfig:normal-entropy}, we see that entropy is minimised at $f=0,1$. Hence, the SC phase is low-entropic.
\end{itemize}

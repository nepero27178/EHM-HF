...
\begin{figure}
	...
	\subfloat[$U$ dependency of $w^{(s)}$ at various $\delta$s.]{
		\includegraphics[width=\linewidth]{/home/nepero27178/Thesis/EHM-HF/Project/plots/refined/Mode=hs/Phase=SC-Singlet/Setup=A[256]/Syms=Ssd/RB=Sd/Obj=HFPs/xVar=U_pVar=V/wd_t=1.0_L=256_δ=Compared_β=100.0.pdf}
...
In singlet channel, the most general gap function is a combination of $s$, $s^*$ and $d$ components. However, whether the actual gap function $\tilde{\Delta}_{\mathbf{k}}$ exhibits all these symmetries is not an immediate question to answer. {\color{tabred}\textbf{Superconductivity in cuprates is observed in $d$-wave channel.}} In Fig.~\ref{fig:sc-data-wd-AB256-compared} we report our findings for the HFP $w^{(d)}$, recalling that the $d$-wave gap is given by:
\[
	\tilde{\Delta}^{(d)} = V w^{(d)}
\]
This gap component is approximately independent of $U$, as is visible in the top panel (Fig.~\ref{subfig:sc-data-wd-A256-compared}), and increases with $V$. Its dependence on doping $\delta$ is much different, as is visible in the bottom panel (Fig.~\ref{subfig:sc-data-wd-B256-compared}). At strong coupling ($V \gtrsim 5$) $w^{(d)}$ decreases with doping, maintaining non-zero values up to almost unitary filling. Note that the data show an hole at $\delta=0.4$. We were unable to understand why the algorithm failed to converge at that point, and fine-tuning of $g$ did not help in this case.

\begin{figure}
	\centering
	\subfloat[$U$ dependency of $w^{(s)}$ at various $\delta$s.]{
		\includegraphics[width=\linewidth]{/home/nepero27178/Thesis/EHM-HF/Project/plots/refined/Mode=hs/Phase=SC-Singlet/Setup=A[256]/Syms=Ssd/RB=Sd/Obj=HFPs/xVar=U_pVar=V/ws_t=1.0_L=256_δ=Compared_β=100.0.pdf}

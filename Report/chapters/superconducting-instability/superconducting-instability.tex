...
\begin{figure}
	\centering
	\subfloat[$U$ dependency of $w^{(s)}$ at various $\delta$s.]{
	...
	\subfloat[$\delta$ dependency of $w^{(s)}$ at various $U$s.]{
	...
	\caption{Dependency of the $s$-wave gap varying the doping $\delta$ and the local repulsion $U$. In the top panel we compare three doping levels $\delta=0.0,~0.2,~0.4$ varying $0 \le U \le 8$. In the bottom panel we compare three repulsion values $U=0.0,~5.0,~10.0$ varying $0 \le \delta \le 0.45$.}
...
\[
	\tilde{\Delta}^{(s)} = -U w^{(s)}
\]
In the conventional HM we know that $s$-wave superconductivity is impossible at HF level. This was discussed in Sec.~\eqref{subsec:sc-hm}. Instead in the EHM there is no selection rule for $w^{(s)}$. The only constraint comes from Eq.~\eqref{eq:sc-wgamma-circle}: $w^{(s)}$ can be non-zero as long as at least one $w^{(\gamma)}$ (with $\gamma=s^*,d$) assumes a suitable value. 
...
\begin{figure}
	\centering
	\subfloat[$V$ dependency of $w^{(d)}$ at various $U$s.]{
	...
	\subfloat[$\delta$ dependency of $w^{(d)}$ at various $U$s.]{
	...
	\caption{Dependency of the $d$-wave gap varying the doping $\delta$ and the local repulsion $U$. In the top panel we compare three doping levels $\delta=0.0,~0.2,~0.4$ varying $0 \le U \le 8$. In the bottom panel we compare three repulsion values $U=0.0,~5.0,~10.0$ varying $0 \le \delta \le 0.45$.}
...
To get a valid understanding of the competition among channels, we need to compare the values of the harmonics $\tilde{\Delta}^{(\gamma)}$ for $\gamma=s,s^*,d$. Competition reflects in the relative ratio between different components. We report a comparison of such in Fig.~\ref{fig:sc-data-surface-wS-B256-surface}. The presented data are limited to the case $U=4$ for graphic reasons, but {\color{tabred}\textbf{data for other values of $U$ are openly accessible at our repository.}}
...
\begin{figure}
	\centering
	\subfloat[$U$ dependency of $t^{(s^*)}$ at various $\delta$s.]{
		\includegraphics[width=\linewidth]{/home/nepero27178/Thesis/EHM-HF/Project/plots/refined/Mode=hs/Phase=SC-Singlet/Setup=A[256]/Syms=Ssd/RB=Sd/Obj=RMPs/xVar=U_pVar=V/tS_t=1.0_L=256_δ=Compared_β=100.0.pdf}
		\label{subfig:sc-data-tS-A256-compared}
	}\\
	\subfloat[$\delta$ dependency of $t^{(s^*)}$ at various $U$s.]{
...
\begin{figure}
	\centering
	\subfloat[$U$ dependency of $t^{(d)}$ at various $\delta$s.]{
		\includegraphics[width=\linewidth]{/home/nepero27178/Thesis/EHM-HF/Project/plots/refined/Mode=hs/Phase=SC-Singlet/Setup=A[256]/Syms=Ssd/RB=Sd/Obj=RMPs/xVar=U_pVar=V/td_t=1.0_L=256_δ=Compared_β=100.0.pdf}
		\label{subfig:sc-data-td-A256-compared}
	}\\
	\subfloat[$\delta$ dependency of $t^{(d)}$ at various $U$s.]{
...
\section{Theoretical constraints on the triplet SC phase HFPs}\label{sec:sc-triplet-constraints}
\todo

\section{Discussion of numeric results (triplet SC)}\label{sec:sc-triplet-numeric}
\todo
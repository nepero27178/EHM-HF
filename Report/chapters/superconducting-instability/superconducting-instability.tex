...
\end{equation}
and $S^{(\mathrm{SC})} \equiv  L_x L_y \times s^{(\mathrm{SC})}$ the total entropy. Here $E_\mathrm{HF}^{(\mathrm{SC})}$ is the ground-state energy for the HF hamiltonian. In the AF phase, we compute $f^{(\mathrm{SC})}$ using the following formula:
\begin{multline}

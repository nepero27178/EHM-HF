...
\begin{align}
	\hat s_\mathbf{k}^x &= \hat \psi_\mathbf{k} + \hat \phi_\mathbf{k}^\dagger \label{eq:sc-hm-x-psi} \\
...
\begin{equation}\label{eq:sc-hm-spin-h}
	...
	\right]
\end{equation}
This operator is similar enough to the one found for the AF phase, in Eq.~\eqref{eq:af-hm-spin-h}. It describes a set of pseudospin subject to a uniform field. As for the AF phase, we define the angles:
\[
...
For the SC phase, diagonalisation is essentially the same procedure as was for the AF phase. Indeed, the diagonalisation matrix is given by
\begin{align}
	W_\mathbf{k} &\equiv e^{-i \theta_\mathbf{k} \tau^y} \nonumber\\
	&= \cos \theta_\mathbf{k} -i \sin \theta_\mathbf{k} \tau^y \nonumber\\
	&= \begin{bmatrix}
...
The Bogoliubov spinors are given by:
\begin{equation}\label{eq:sc-Gamma=WPsi}
	\hat\Gamma_\mathbf{k} = W_\mathbf{k} \hat \Psi_\mathbf{k} = \begin{bmatrix}

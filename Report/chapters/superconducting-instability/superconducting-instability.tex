...
\end{align}
whose EVs are relevant for the SC phase.
...
\begin{equation}\label{eq:sc-bogoliubov-angle-relations}
	\sin 2\tilde{\xi}_\mathbf{k} = \frac{\Re\{\tilde{\Delta}_\mathbf{k}\}}{|\tilde{\Delta}_\mathbf{k}|}
	\quad
	\cos 2\tilde{\xi}_\mathbf{k} = \frac{\Im\{\tilde{\Delta}_\mathbf{k}\}}{|\tilde{\Delta}_\mathbf{k}|}
	\quad
...
When it comes to the Fock RWCs, being the SC phase density-uniform we can re-use almost the entirety of the derivation of Sec.~\ref{sec:normal-fock-bands-renormalization}. Setting as in Eqns.~\eqref{eq:normal-uqsigma-def}, \eqref{eq:af-uqsigma-def},
...
\]
which essentially states translational invariance and spin degeneracy. This assumption is obviously well-verified by the BCS ground state of Eq.~\ref{appeq:sc-hm-gs}. Recognising the symmetry decomposition
\[
...
\]
which is coherent with our requirement of a uniform-density, spin uniform and inversion symmetric phase. From the above results we have a simple expression for the system density,
\[
\begin{aligned}
	n &= \frac{N}{2L_x L_y} \\
	&= \frac{1}{2L_x L_y} \sum_\sigma \sum_{\mathbf{k} \in \mathrm{BZ}} u_{\mathbf{k}\sigma} \\
...
\end{align}
where $\tau^+ \equiv (\tau^x + i \tau^y)/2$. We used Eqns.~\eqref{eq:sc-x-expectation-non-local}, \eqref{eq:sc-y-expectation-non-local} and \eqref{eq:sc-rot-z-expectation} as well as the thermal factor defined in Eq.~\eqref{eq:sc-ren-thermal-factors-def}. The derivation above holds for all symmetries, including the $s$ one as testifies Eq.~\eqref{appeq:sc-hm-ws-self-consistency-2}. Explicitly, five self-consitency equations are then derived and listed in Tab.~\ref{tab:sc-ehm-wgamma-self-consistencies}. 
...
\end{aligned}
\]
having we used the self-consistency equations of Tab.~\ref{tab:sc-ehm-wgamma-self-consistencies}. This proves Eq.\eqref{eq:sc-ehm-gap-energy}, which is pretty similar to Eq.~\eqref{eq:af-ehm-gap-energy} with one important exception. Since the right-hand side of Eq.\eqref{eq:sc-ehm-gap-energy} is positive by definition, for $V>0$ we have the constraint
\[
	\sum_{\gamma} \big| w^{(\gamma)} \big|^2 \ge M
	\qq{being}
	M^{(\mathrm{SC})} \equiv \big| w^{(s)} \big|^2 \frac U V
\]
Now, as we state in Sec.~\ref{appsubsec:sc-hm-impossible}, for the pure HM we have identically $w^{(s)}=0$. The pure HM cannot express BCS-like superconductivity. For the EHM instead we see that:
\begin{itemize}
	\item In the strong coupling limit $U \ll V$, even small $s$-wave gaps impose larger $\gamma \neq s$ components;
	\item The $s$-wave gap $w^{(s)}$ cannot rise without the contributions of more complex gap topologies, which translates in (with an uncivil abuse of notation)
	\[
		\bigotimes_{\gamma \neq s}
		\lim_{w^{(\gamma)}\to0} 
		w^{(s)} = 0

...
We proceed here as we did for the AF phase in Sec.~\ref{subsec:af-hm}. We start by giving a matricial representation of the approximated hamiltonian, then map it to a pseudospin problem and finally specify how to diagonalise it to retrieve its ground state.

\paragraph{Matricial representation.}

The Hubbard hamiltonian is approximated by:
\begin{multline}
...
\]
Notice we are using the renormalised chemical potential. 

From Fermi anticommutation rules we have:
\[
	\hat n_{\mathbf{k}\uparrow} + \hat n_{-\mathbf{k}\downarrow} = \hat c_{\mathbf{k}\uparrow}^\dagger \hat c_{\mathbf{k}\uparrow} - \hat c_{-\mathbf{k}\downarrow} \hat c_{-\mathbf{k}\downarrow}^\dagger + 1
\]
\textbf{FROM HERE}
...
\begin{aligned}
	...
	\right] \\
	&= \sum_{\mathbf{k} \in \mathrm{BZ}} \xi_\mathbf{k} \hat s_\mathbf{k}^z + E_\mathrm{a}^{(\mathrm{SC})}
\end{aligned}
\]
being $E_\mathrm{a}^{(\mathrm{SC})}$ the ``anticommutation energy'' coming from the kinetic term,
\begin{equation}\label{appeq:sc-hm-anticommutation-energy}

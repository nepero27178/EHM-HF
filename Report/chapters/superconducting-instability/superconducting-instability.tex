...
This chapter is devoted to studying the superconducting (SC) phase of the system. Superconductivity is obtained essentially by allowing for Cooper fluctuations, obtaining then a phase much different from normal and AF. The conventional, locally repulsive HM, cannot exhibit a BCS-like superconducting phase, an argument discussed in Sec.~\ref{appsubsec:sc-hm-impossible}. The $2$D EHM, supplied with non-local attraction, fixes this and makes a SC phase possible, by the means of a slightly more complex Cooper pairing mechanism. Not only, various SC topological classes are possible based on the competition between isotropic and anisotropic pairing. Finally, spin pairing algebra plays a role in the competition defining singlet and triplet pairing channels.

The most interesting effects with respect to conventional HM are the insurgence of a BCS-like superconducting phase, as well as the already discussed bands renormalization and resymmetrization and the extension of the gap function symmetry structure.

\section{The SC phase and its symmetries}\label{sec:sc-phase-theory}

The starting point is, as done for the other phases, the discussion of the symmetries of the SC phase. The BCS-like phase we are dealing with is described by the full $\mathrm{U}^\mathrm{c}(1)$ SSB. All other symmetries are preserved, but as for the normal and AF phases we allow for 
\[
	\mathrm{C}_4 \SSB \mathrm{C}_2
\]
without breaking inversion symmetry. Therefore, being translational invariance fully preserved, the system density we assume to be uniform across all sites, similarly to the normal phase discussed in Chap.~\ref{chap:normal-phase}.
...
\begin{aligned}
	...
	&&\text{Hartree, Fock and Cooper contractions} \\
\end{aligned}	
\]
We anticipate the role of all RWCs in Tab.~\ref{tab:sc-RWCs}.
...
\begin{table}[t]
	...
	\begin{tabular}{c c c l}
		...
		\midrule
		\multirow{5}{*}{$\hat H_V$} & \multirow{2}{*}{o.s.} & Hartree & $\mu$ shift by $nzV$ \\
...
\begin{multline*}
	\hat H_t + \hat H_U \stackrel{\mathrm{MFT}}{\simeq} \sum_{\mathbf{k} \in \mathrm{BZ}} \epsilon_\mathbf{k}
	\left[
	...
	\right]
\end{multline*}
with $E_{\mathrm{H}/U}^{(\mathrm{SC})}$, $E_{\mathrm{C}/U}^{(\mathrm{SC})}$ the contraction energies arising from Hartree and Cooper channels, and $E_\mathrm{a}^{(\mathrm{SC})}$ the energy shift resulting from anticommutation rules, given respectively in Eq.~\eqref{appeq:sc-hm-hartree-U-Ec}, \eqref{appeq:sc-hm-hartree-C-Ec}, and \eqref{appeq:sc-hm-anticommutation-energy}.

In deriving this result we decomposed the interaction of the intermediate passage of Eq.~\eqref{appeq:sc-hm-U-int-2} and assumed the Cooper pair to have null total momentum. Moreover, we recognized in the passage of Eq.~\eqref{appeq:sc-hm-U-int-3} the presence of a $s$-wave harmonic transform of the function $w_\mathbf{q}$, and thus defined in Eq.~\eqref{appeq:sc-hm-ws-self-consistency},
\[
...
Nambu-Bogoliubov representation of the MFT hamiltonian is given in Eq.~\eqref{appeq:sc-hm-bogoliubov-h} and equivalently in Eq.~\eqref{appeq:af-sc-spin-h},
\[
...
\subsection{Hartree channel in the SC phase: analytic derivation}\label{subsec:sc-hartree-analytic}

The Hartree fluctuations are density-density couplings in the MFT hamiltonian. Knowing the deal with a uniform-density phase, we can re-use as it is the derivation of Sec.~\ref{subsec:normal-hartree-analytic}. Without repeating the calculations, the $\hat H_V$ is reduced in the Hartree channel to
\[
...
As done for the other phases, we here include rapidly the bands renormalization effect derived from the Fock sector of the non-local attraction. This discussion is essentially analogous the one done for the normal phase in Sec.~\ref{sec:normal-fock-bands-renormalization}.
...
\]
which is coherent with our requirement of a uniform-density, spin uniform and inversion symmetric phase. Moreover,
\[
\begin{aligned}
...
\section{Cooper effects: anisotropic pairing}\label{sec:sc-cooper}

Anisotropic pairing in superconducting fluctuations is induced by the Cooper RWCs. The two sectors of $\hat H_V$ (o.s., s.s.) are Fourier-transformed respectively in Eqns.~\eqref{eq:HV-os-fourier}, \eqref{eq:HV-ss-fourier}. In Tab.~\ref{tab:sc-pairings-symmetry-structures} we identified the correct RWCs for singlet and triplet pairing channels, noting that the o.s. sector contributes to both while the s.s. to just the triplet channel.

\subsection{Cooper o.s. channel in the SC phase: analytic derivation}\label{subsec:sc-cooper-os-analytic}

This section deals with Cooper fluctuations induced by the o.s. part of the non-local hamiltonian --referring to the notation of Eq.~\eqref{eq:ehm-non-local-ss-os-terms}-- somewhat the simplest form of Cooper pairing. This sector contributes both to singlet pairing and triplet pairing. Considering Cooper fluctuations in the singlet channel, we need to break $\mathrm{U}^\mathrm{c}(1)$ symmetry imposing space inversion symmetry, while for the triplet anti-symmetry is required. Set as in Eq.~\eqref{eq:sc-wq-def} the EV
\[
	w_\mathbf{k} = \langle
		\hat \phi_\mathbf{k}^\dagger
	\rangle
...
\[
	w^{(s)} \equiv \frac{1}{L_x L_y} \sum_{\mathbf{k}\in\mathrm{BZ}} w_\mathbf{k}
\]
which reduces Eq.~\eqref{eq:sc-U-cooper-int1} to
\begin{equation}\label{eq:sc-U-cooper-int2}
	\hat H_U \simeq \text{(Hartree channel)} + U \sum_{\mathbf{k}\in\mathrm{BZ}} \left[
		w^{(s)} \hat \phi_\mathbf{k} + w^{(s)*} \hat \phi_\mathbf{k}^\dagger
	\right] - E_{\mathrm{C}/U}^{(\mathrm{SC})}
\end{equation}
...
Consider Eq.~\eqref{eq:HV-os-fourier}, and perform Wick contractions in the Cooper channel, giving
...
\begin{multline*}
	...
	\Big]
\end{multline*}
Identical considerations as above hold, and just the $\mathbf{K}=\mathbf{0}$ term contributes.:
\begin{multline*}
	\hat H_V^{(\mathrm{o.s.})} \simeq \text{(Hartree channel)} \\
	-\frac{2V}{L_x L_y}
...
Recognizing our superconducting order parameter in the various EVs, we get
...
\end{equation}
We also made use of the harmonics decomposition of Eq.~\eqref{eq:coscos-symmetries-decomposition}. In the above equation, the contraction energy is given by
...
For the $\gamma$-wave component of the superconducting order parameter, the following chain, similar to Eq.~\eqref{appeq:sc-hm-ws-self-consistency-2}, holds:
\begin{align}
...
\begin{table}
	...
	\begin{tabular}{r E l l}
		\textbf{Symmetry} & \multicolumn{2}{c}{\textbf{Self-consistency equation}} & \textbf{Graph} \\
		\midrule
		$s$-wave & $w^{(s)}$ & $\displaystyle \frac{1}{L_x L_y} \sum_{\mathbf{k}\in\mathrm{BZ}} \frac{\tilde{\Delta}_\mathbf{k}}{\tilde{E}_\mathbf{k}} \tanh(\frac{\beta\tilde{E}_\mathbf{k}}{2}) $ & Fig.~\ref{subfig:s-wave-correlator} \\[1.5em]
		$s^*$-wave & $w^{(s^*)}$ & $\displaystyle \frac{1}{L_x L_y} \sum_{\mathbf{k}\in\mathrm{BZ}} (\cos k_x + \cos k_y) \frac{\tilde{\Delta}_\mathbf{k}}{\tilde{E}_\mathbf{k}} \tanh(\frac{\beta\tilde{E}_\mathbf{k}}{2}) $ & Fig.~\ref{subfig:s*-wave-correlator} \\[1.5em]
		$p_x$-wave & $w^{(p_x)}$ & $\displaystyle \frac{i\sqrt{2}}{L_x L_y} \sum_{\mathbf{k}\in\mathrm{BZ}} \sin k_x \frac{\tilde{\Delta}_\mathbf{k}}{\tilde{E}_\mathbf{k}} \tanh(\frac{\beta\tilde{E}_\mathbf{k}}{2}) $ & Fig.~\ref{subfig:px-wave-correlator} \\[1.5em]
		$p_y$-wave & $w^{(p_y)}$ & $\displaystyle \frac{i\sqrt{2}}{L_x L_y} \sum_{\mathbf{k}\in\mathrm{BZ}} \sin k_y \frac{\tilde{\Delta}_\mathbf{k}}{\tilde{E}_\mathbf{k}} \tanh(\frac{\beta\tilde{E}_\mathbf{k}}{2}) $ & Fig.~\ref{subfig:py-wave-correlator} \\[1.5em]
		$d_{x^2-y^2}$-wave & $w^{(d)}$ & $\displaystyle \frac{1}{L_x L_y} \sum_{\mathbf{k}\in\mathrm{BZ}} (\cos k_x - \cos k_y) \frac{\tilde{\Delta}_\mathbf{k}}{\tilde{E}_\mathbf{k}} \tanh(\frac{\beta\tilde{E}_\mathbf{k}}{2}) $ & Fig.~\ref{subfig:d-wave-correlator} \\
		\midrule
	\end{tabular}
...
\subsection{Cooper o.s. contraction energy}\label{subsec:sc-ehm-cooper-os-energy}

The Cooper contraction energy is immediately derived from Eq.~\eqref{eq:sc-Vos-cooper-double-counting} by recognizing the harmonics of $w_\mathbf{k}$,
\[
	E_{\mathrm{C}/V/\mathrm{o.s.}}^{(\mathrm{SC})} = -L_x L_y \times V \sum_\gamma \big|
		w^{(\gamma)}
	\big|^2
\]
...
\]
discussed for the conventional HM in Eq.~\eqref{appeq:sc-hm-hartree-C-Ec}.
...
%	\hat c_{i\sigma}^\dagger \hat c_{j\sigma}
%	\rangle \hat c_{j\sigma}^\dagger  \hat c_{i\sigma} + \hc
%	\right]	= \frac{2V}{L_x L_y} \sum_{\mathbf{k}, \mathbf{k}'} \sum_\sigma \left[
...
%	\hat c_{\mathbf{k}'\sigma}^\dagger  \hat c_{\mathbf{k}'\sigma}
%\end{equation}
%Recall Eq.~\eqref{eq:coscos-symmetries-decomposition}: the structure term decomposes in harmonic waves, and this feature is particularly handy in order to decouple the $\mathbf{k}$ and $\mathbf{k}'$ parts, as we later do.
%
%{\color{tabred}
%	
...
%	\rangle \hat c_{j\sigma}^\dagger  \hat c_{i\sigma} + \hc
%	\right]
%	= \frac{2V}{L_x L_y} \sum_{\mathbf{k}, \mathbf{k}'} \left[
%	\cos \left( \delta k_x \right) + \cos \left( \delta k_y \right)	
%	\right] \\
%	\times \bigg[
%		\left(
...
%		\hat c_{\mathbf{k}'\uparrow}^\dagger \hat c_{\mathbf{k}'\uparrow} \\
%		+ \left(
%			 1 - \cos^2 \theta_{-\mathbf{k}} \tanh(\frac{\beta E_{-\mathbf{k}}}{2}) - f\left( E_{-\mathbf{k}}; \beta,0 \right)
%		\right)
%		\hat c_{\mathbf{k}'\downarrow}^\dagger \hat c_{\mathbf{k}'\downarrow}
%	\bigg]
%	\label{eq:reciprocal-space-non-local-interaction-fock-dd-intermediate-sc-2}
%\end{multline}
%Spatial inversion symmetry guarantees
%\[
%	\cos^2 \theta_{-\mathbf{k}} = \cos^2 \theta_\mathbf{k}
%	\qq{as well as}
%	E_{-\mathbf{k}} = E_\mathbf{k}
%\]
...
%\[
%\begin{aligned}
%	1 - \cos^2 \theta_\mathbf{k} \tanh(\frac{\beta E_\mathbf{k}}{2}) - f\left( E_\mathbf{k}; \beta,0 \right) &= \sin^2 \theta_\mathbf{k} \tanh(\frac{\beta E_\mathbf{k}}{2}) + 1 - \tanh(\frac{\beta E_\mathbf{k}}{2}) - f\left( E_\mathbf{k}; \beta,0 \right) \\
%	&= \sin^2 \theta_\mathbf{k} \tanh(\frac{\beta E_\mathbf{k}}{2}) + f\left( E_\mathbf{k}; \beta,0 \right)
%\end{aligned}
%\]
%Since
...
%\[
%\begin{aligned}
%	\sin^2 \theta_\mathbf{k} &= \frac{1 - \cos(2\theta_\mathbf{k})}{2} \\
%	&= \frac 1 2 - \frac{\xi_\mathbf{k}}{2\sqrt{\xi_\mathbf{k}^2 + \abs{\Delta_\mathbf{k}}^2}}
%\end{aligned}
%\]
%and let:
%\[
%	g_\mathbf{k} \equiv \left[ 
%		\frac 1 2 - \frac{\xi_\mathbf{k}}{2\sqrt{\xi_\mathbf{k}^2 + \abs{\Delta_\mathbf{k}}^2}}
%	\right] \tanh(\frac{\beta}{2} \sqrt{\xi_\mathbf{k}^2 + \abs{\Delta_\mathbf{k}}^2}) + f\left(\sqrt{\xi_\mathbf{k}^2 + \abs{\Delta_\mathbf{k}}^2}; \beta,0 \right)
%\]
%which further simplifies to
%\begin{equation}\label{eq:sc-bands-renormalization-gq-definition}
%	g_\mathbf{k} \equiv
%	\frac 1 2 - \frac{\xi_\mathbf{k}}{2\sqrt{\xi_\mathbf{k}^2 + \abs{\Delta_\mathbf{k}}^2}}
%	\tanh(\frac{\beta}{2} \sqrt{\xi_\mathbf{k}^2 + \abs{\Delta_\mathbf{k}}^2})
%\end{equation}
%
...
%		\cos \left( \delta k_x \right) + \cos \left( \delta k_y \right)	
%	\right] g_\mathbf{k} \hat c_{\mathbf{k}'\sigma}^\dagger \hat c_{\mathbf{k}'\sigma}
%\]
...
%	\frac{2V}{L_x L_y} \sum_{\mathbf{k}, \mathbf{k}'} \sum_\sigma \left[
%	\cos \left( \delta k_x \right) + \cos \left( \delta k_y \right)	
%	\right] g_\mathbf{k} \hat c_{\mathbf{k}'\sigma}^\dagger \hat c_{\mathbf{k}'\sigma} = \frac{V}{L_x L_y} \sum_{\mathbf{k}, \mathbf{k}'} \sum_{\sigma,\gamma} \varphi_\mathbf{k}^{(\gamma)} \varphi_{\mathbf{k}'}^{(\gamma)*} g_\mathbf{k} \hat c_{\mathbf{k}'\sigma}^\dagger \hat c_{\mathbf{k}'\sigma}
%\end{equation}
%Evidently from Eq.~\eqref{eq:sc-bands-renormalization-gq-definition}, $g_\mathbf{k}$ is an even function of the momentum, a feature that ensures
%\[
%	\frac{1}{L_xL_y} \sum_\mathbf{k} \varphi_\mathbf{k}^{(p_\ell)} g_\mathbf{k} = 0
...
%\[
%	g^{(s^*)} \equiv \frac{1}{L_xL_y} \sum_\mathbf{k} (\cos q_x + \cos q_y) g_\mathbf{k} 
%	\qquad
%	g^{(d)} \equiv \frac{1}{L_xL_y} \sum_\mathbf{k} (\cos q_x - \cos q_y) g_\mathbf{k}
%\]
...
%	\cos \left( \delta k_x \right) + \cos \left( \delta k_y \right)	
%	\right] g_\mathbf{k} \hat c_{\mathbf{k}'\sigma}^\dagger \hat c_{\mathbf{k}'\sigma} \\
%	= V \sum_{\mathbf{k}\sigma} 
%	\left[
%		g^{(s^*)} (\cos q_x + \cos q_y) + g^{(d)} (\cos q_x - \cos q_y)
%	\right] \hat c_{\mathbf{k}\sigma}^\dagger \hat c_{\mathbf{k}\sigma} \label{eq:reciprocal-space-non-local-interaction-fock-dd-intermediate-sc-4}
%\end{multline}
...
%
%As long as we work in a precise sector ($s\oplus s^*$ as well as $d_{x^2-y^2}$) with a gap function completely contained in said symmetry sector, we can rule out the $d_{x^2-y^2}$-wave component of $g_\mathbf{k}$ from Eq.~\eqref{eq:sc-bands-renormalization-general}
%\[
%	\frac{1}{L_xL_y} \sum_\mathbf{k} \varphi_\mathbf{k}^{(d)} g_\mathbf{k} = 0
%\]
%The result above is valid only if $\Delta_\mathbf{k}$ is described only by a single symmetry harmonics. This is because under a rotation of angle $\pi/2$ the two functions behave differently:
%\[
%	(q_x, q_y) \to (-q_y, q_x)
%	\quad\colon\quad
%	\begin{cases}
%		\varphi_\mathbf{k}^{(d)} \to - \varphi_\mathbf{k}^{(d)} \\
%		g_\mathbf{k} \to g_\mathbf{k}
%	\end{cases}
%\]
...
\subsection{Cooper s.s. channel in the SC phase: analytical considerations}\label{subsec:sc-cooper-ss-analytic}

Consider Eq.~\eqref{eq:HV-ss-fourier} in the Cooper channel,
\begin{multline*}
	\hat H_V^{(\mathrm{s.s.})} \simeq \text{(Hartree channel)} + \text{(Fock channel)}\\
	...
	\times \Big[ 
		\big\langle
		\hat c_{\mathbf{K}+\mathbf{k} \sigma}^\dagger \hat c_{\mathbf{K}-\mathbf{k} \sigma}^\dagger
		\big\rangle 
		\hat c_{\mathbf{K}-\mathbf{k}' \sigma} \hat c_{\mathbf{K}+\mathbf{k}'\sigma} \\
		+
		\hat c_{\mathbf{K}+\mathbf{k} \sigma}^\dagger \hat c_{\mathbf{K}-\mathbf{k} \sigma}^\dagger
		\big\langle 
		\hat c_{\mathbf{K}-\mathbf{k}' \sigma} \hat c_{\mathbf{K}+\mathbf{k}'\sigma}
	...
	\Big]
\end{multline*}
In principle, this pairing channel is legitimate and gives rise to ``polarized'' (same spin) Cooper class RWCs. This kind of spin pairing couples with antisymmetric spatial structures, working in the triplet pairing channel. However, we can safely ignore and set to zero these EVs, as is proven \cite{senarath2025mixed}, \cite{hutchinson2021mixed}. The reason is simply that the superconducting phase preserves $\mathrm{SU}(2)$ spin symmetry, thus all three triplet pairing subspaces
\[
	\text{$z$ spin projection} \in \lbrace -1, 0, 1 \rbrace
\]
are degenerate and give rise to an identical solution. We can safely use the most convenient to us, the one with null spin projection, which can be mapped on the HM model of superconductivity discussed in Sec.~\ref{appsec:superconducting-phase}.

Anyhow, for the sake of completeness, the s.s. pairing channel is explicitly discussed in Sec.~\mref. As is seen there, following the sketch of \cite{hutchinson2021mixed}, the resulting triplet of HFPs can be safely reduced to the simplified set of o.s. triplet HFPs discussed in previous sections.
...
\subsection{The relative phase in the zero temperature half-filled ground state}\label{subsec:sc-zero-T-half-filling-gs}

The conventional BCS ground state is widely known and derived for HM superconductivity in Sec.~\ref{appsubsec:sc-zero-T-half-filling-gs}. Jumping to Eq.~\eqref{appeq:sc-hm-gs} and extending it to the present context of the EHM, we get
\[
...
\end{aligned}
\]
results that are coherent with Eqns.~\eqref{eq:sc-x-expectation-non-local} as well as \eqref{eq:sc-y-expectation-non-local} and allow us to identify the phase with the pseudofield $xy$ angle.

\section{SC free energy density}\label{sec:sc-free-energy}

Free energy density of the superconducting phase is derived as for the normal phase in Sec.~\ref{sec:normal-free-energy}. As for the AF phase of Sec.~\ref{sec:af-free-energy}, two fermionic bands are present; however, recalling the antiferromagnet entropy density of Eq.~\eqref{eq:af-density-entropy}, the key differences are here given by the fact that the chemical potential inside the Fermi-Dirac factors is null and the sum is extended on the full BZ (but not later on spin index),
\begin{multline}
...
\[
	...
	s \equiv s^{(-)} + s^{(+)} = 2 s^{(+)}
\]
being $s$ the total entropy density.

The five contributions to free energy are:
\begin{enumerate}
	...
	\begin{aligned}
		...
		&= \frac{1}{L_x L_y} \sum_{\mathbf{k} \in \mathrm{MBZ}} \tilde{\xi}_\mathbf{k}
	\end{aligned}
	\]

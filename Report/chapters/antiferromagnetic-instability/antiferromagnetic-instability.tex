...
In this phase, HF methods give two main self-consistent effects. First, the presence of the non-local attraction redefines the bands gap, an effect we denote ``gap complexification'' (Sec.~\ref{sec:af-full-hamiltonian}) and later show is actually suppressed (Sec.~\ref{sec:af-constraints}).  Second, bands shrinking (Sec.~\ref{sec:af-fock-bands-renormalization}) similarly to what found for the normal phase.
...
The Ansats of Eq.~\eqref{eq:af-mean-field-ansatz} breaks translational invariance in each spin sector, but preserves $\mathrm{U}^z(1)$ and $\mathrm{U}^c(1)$ symmetries. Let us prove this. The Ansatz is defined by the following EV being non-zero,
...
Before extending to the non-local attraction, we dedicate the end of this section to complete the theoretical description of antiferromagnetism in the conventional, locally repulsive Hubbard model. Indeed, this thesis aims to investigate how $\hat H_V$ changes the various phases with respect to the pure HM.

\subsection{Local Hartree channel in the normal phase: analytic derivation}\label{subsec:af-local-hartree-analytic}

The HF method, applied to a quartic operator, gives the decomposition of Eq.~\mref. Decompose $\hat H_U$:
\[
...
\end{align}
Note that $\hat n_{\mathbf{k}\sigma}$ is not the Fourier-transform of $\hat n_{\mathbf{r}\sigma}$. Collecting everything, the local interaction is reduced to
\[
...
\subsection{Local Hartree contraction energy}\label{subsec:af-local-hartree-energy}
...
In this section we give a description of antiferromagnetism in the conventional HM, before moving later to the EHM. This is useful in order to identify clearly what changes due to $\hat H_V$. 
...
\]
In therms of the spinors $\hat \Psi_{\mathbf{k}\sigma}$, the kinetic operator can be written as
\[
...
Collecting everything, we get the following representation of the AF hamiltonian for the conventional HM:
...
\begin{align}
	\ev{\hat s_{\mathbf{k}\sigma}^x} &= \frac{\Re\lbrace\Delta_{\mathbf{k}\sigma}\rbrace}{E_{\mathbf{k}\sigma}} \mathrm{Th}_{\mathbf{k}\sigma}^{(-)}[\beta,\mu] \label{appeq:af-hm-sx}\\
	\ev{\hat s_{\mathbf{k}\sigma}^y} &= \frac{\Im\lbrace\Delta_{\mathbf{k}\sigma}\rbrace}{E_{\mathbf{k}\sigma}}  \mathrm{Th}_{\mathbf{k}\sigma}^{(-)}[\beta,\mu] \label{appeq:af-hm-sy}\\
	\ev{\hat s_{\mathbf{k}\sigma}^z} &=  -\frac{\epsilon_{\mathbf{k}\sigma}}{E_{\mathbf{k}\sigma}}  \mathrm{Th}_{\mathbf{k}\sigma}^{(-)}[\beta,\mu] \label{appeq:af-hm-sz}
\end{align}
...
\]
and the following relations hold (\textit{tilde} sign indicates the renormalized quantity):
\begin{align}
...
\end{align}
equivalent to the ones for the pure HM, Eqns.~\eqref{appeq:af-hm-sx}, \eqref{appeq:af-hm-sy} and \eqref{appeq:af-hm-sz}. Moving to the tilted frame, the $z$ component of the pseudo-spin is just a thermal superposition of ``up'' and ``down'' states
...
\begin{equation}\label{eq:af-ren-thermal-factors-def}
	...
	\right)
\end{equation}
Angles satisfy: \begin{equation}\label{eq:af-bogoliubov-angle-relations}
...
\end{equation}
Physical behaviour is the same as for the pure HM. In terms of MFT discussion, the AF phase is similar enough to the normal phase of Chap.~\ref{chap:normal-phase}: RWCs are the same of Sec.~\ref{sec:normal-phase-theory}, listed in Tabs.~\ref{tab:U-wick-terms-phases} and \ref{tab:V-wick-terms-phases}: all three Hartree contractions, responsible in normal phase for chemical potential shift, and the s.s. Fock contraction, that originated the hopping renormalization. Let us start with Hartree terms: due to the ``staggered'' nature of commensurate AF phase, their role is here more complex. Then we will move to the Fock term, discussing MFT approximations effects on bands. Next sections are basically an extension of the calculations of Chap.~\ref{chap:normal-phase}.
...
\begin{equation}\label{eq:af-hartree-renormalization-density}
	...
	\right]
\end{equation}
The discussion is identical to Sec.~\ref{subsec:normal-hartree-analytic} for the normal phase, with renormalization of $\mu$ being the same of Eq.~\eqref{eq:normal-mu-shift}.
\begin{equation}\label{eq:af-mu-shift}
	\tilde{\mu} \equiv \mu + n(2zV-U)
\end{equation}
Within this equation we are also already implicitly considering the $\mu$ shift effect due to $\hat H_U$., discussed both in Sec.~\ref{sec:normal-hartree-mu} (and Sec.~\ref{appsubsec:af-hartree-channel}, specifically for antiferromagnetism in conventional HM).

\paragraph{Magnetization terms.}

When it comes to magnetization, $\mathrm{s.s.}$ and $\mathrm{o.s.}$ terms of Expr.~\eqref{eq:af-ehm-hartree-int-1} perfectly cancel out:
\[
	mV \sum_{\ev{\mathbf{r}\mathbf{r}'}} \sum_\sigma (-1)^{x+y+\delta_{\sigma=\uparrow}} \left[
		\left(
			\hat n_{\mathbf{r}\sigma} - \hat n_{\mathbf{r}'\sigma}
	...
	\right] = 0
\]
thus Hartree RWCs only affects $\mu$. This is somewhat to be expected: in the HM context, gap function emerges in its purely $s$-wave structure from this precise computation on Hartree terms. However, $\hat H_V$ non-local interaction cannot provide a pure $s$-wave potential component. Effects on the gap function are to be observed somewhere else.
...
\end{multline*}
Evidently the mixed terms (those of order $\mathcal{O}(n)\times\mathcal{O}(m)$) cancel out, and since sign factors necessarily alternate on neighboiring sites, the remainder is just
\[
	V \sum_{\ev{\mathbf{r}\mathbf{r}'}} \sum_\sigma \langle
		\hat n_{i\sigma}
	\rangle \langle
...
\[
	...
	\big| \ge 0
\]
Because of this, for conventional HM, AF ground state is a degenerate Fermi gas of $\gamma^{(\pm)}$ quasiparticles, and for null temperature acquires the simple form of Eq.~\eqref{appeq:af-hm-gs-4}. Assuming the new ground state to be similar and going back to Eq.~\eqref{eq:af-V-fock-int1}, this implies only two contributions are non-zero:
\[
	\mathbf{k} = -\mathbf{k}'
	\qq{or}
	\mathbf{k}+\bm{\pi} = -\mathbf{k}'
\]
The first case gives us normal fermionic EVs, $\langle \hat n_{\mathbf{k}\sigma} \rangle$; the second, staggered AF order parameter: $\langle \hat \psi_{\mathbf{k}\sigma} \rangle$. Then Eq.~\eqref{eq:af-V-fock-int1} is reduced to:
\begin{multline}
...
Diagonal terms of Eq.~\eqref{eq:af-V-fock-int2} are simple density interactions with mean density field. Consider Fig.~\ref{subfig:Kk-qq'-generic}: density at vector $\mathbf{q}\equiv\mathbf{K}+\mathbf{k}$ interacts with mean density at $\mathbf{q}'\equiv\mathbf{K}-\mathbf{k}$. These variables are depicted in Fig.~\ref{subfig:Kk-qq'-newvariables}. Apply in the diagonal part of Eq.~\eqref{eq:af-V-fock-int2} this variables change (which is the same of Eq.~\eqref{eq:normal-V-fock-int3}, represented in Fig.~\ref{fig:Kk-qq'})
\begin{align}
...
\]
Renormalized bands need to be symmetric functions of the moment. Hence for $\gamma\in\lbrace p_x, p_y \rbrace$ the above sum vanishes. The remaining $s^*$ and $d$ symmetries are legitimate HFPs with relative self-consistency equations, reported in Tab.~\ref{tab:af-ehm-ugamma-self-consistencies} using spin degeneracy. 
...
Consider off-diagonal terms of Eq.~\eqref{eq:af-V-fock-int2}. These contribute instead to full gap, being out of diagonal in the $2\times2$ hamiltonian matrix of Eq.~\eqref{eq:af-hk}. In pseudo-spin picture, they account for a transverse field. Define $\mathbf{q}$, $\mathbf{q}'$ as in Fig.~\ref{subfig:Kk-qq'-newvariables}, 
\[
...
\end{align}
where we made use of Eq.~\eqref{eq:af-ehm-vphi-psi-property}, Tab.~\ref{tab:af-pseudo-spin-map} and Eq.~\eqref{eq:af-y-expectation-non-local}. There is no particular reason to exclude one component or another. In Tab.~\ref{tab:af-ehm-vgamma-self-consistencies} the four derived self-consistency equations are reported (already ignoring spin index).
...
From structure factors definitions of Tab.~\ref{tab:x-wave-reciprocal-factors}, it's immediate to see that:
\[
...
\begin{equation}\label{eq:af-V-fock-double-counting-int3}
	...
	\right]
\end{equation}
Interestingly enough, the new HFPs amplitudes contribute positively to contraction energy. This fact is non-trivial and has consequences we will discuss in Sec.~\ref{subsec:af-gap-energy}.
...
For the normal phase, we saw that when $\mathrm{C}_4$ symmetry is preserved (Sec.~\ref{subsec:normal-bands-shrinking}) we can conclude immediately that bands renormalization is actually a shrinking. An identical result holds here: if $\mathrm{C}_4$ symmetry is imposed onto the renormalized bands,
\[

...
This chapter is devoted to an in-depth discussion of the antiferromagnetic (AF) long-range ordering in the EHM by the means of MFT. In the context of the $2$D HM, discussed in App.~\ref{appendix:mean-field-hubbard}, AF ordering establishes as an unconventional metallic phase only stable at half-filling (as is discussed in Sec.~\ref{appsubsec:af-instability}). In EHM, the situation is similar: the present analysis is carried out based on the argument that, given the symmetry structure of the non-local NN interaction, the fundamental features of commensurate AF in HM (the ability to establish antiferromagnetism as a two-sites periodic SDW) are effectively preserved, while parameters are redefined by interaction itself. The first effect we discuss in this chapter is ``gap complexification'', in Sec.~\ref{sec:af-full-hamiltonian}; as it will turn out, such effect arises from MFT but is later suppressed. The second is combined bands renormalization-resymmetrization (both in Sec.~\ref{sec:af-fock-bands-renormalization}), the latter being a general feature of EHM common to all phases preserving (or, at most, weakening without destroying) translational invariance, previously discussed in Sec.~\ref{sec:normal-fock-bands-renormalization}.

\section{The AF phase and its symmetries}\label{sec:af-phase-theory}

To start, AF phase is not uniquely defined; actually, there exists a variety of ways to establish a SDW phase in a spin-balanced square lattice, a system where both spin sectors are equally populated, but differently displaced. Fig.~\ref{fig:antiferromagnets} presents three (of many) types of antiferromagnets. Left panel shows the one we deal with: SDW structures are $2$-sites periodic, making the AF phase ``commensurate''. The central one presents another simple commensurate AF with higher periodicity. The right panel shows a possible ``striped'' antiferromagnet, where AF is effectively established along one particular direction; the example we report is $2$ sites periodic in $y$ direction, but any other periodicity could work.
%
\begin{figure}
	\centering
...
\end{aligned}
\]
We describe a SDW whose period is of two sites, as in the left panel of Fig.~\ref{fig:antiferromagnets}. As done for the normal phase, we allow theoretically
\[
	\mathrm{C}_4 \SSB \mathrm{C}_2
\]
in order to capture the same possible nematic effects seen in Sec.~\ref{subsec:normal-fock-analytic}. The theoretical reason to do so for AF phase relies in our previous discussion. As said, planar Hubbard lattices exhibits tendency towards nematic magnetic ordering, thus (even if our Ansatz is the simplest and non evidently nematic) nematic effects are not to be excluded \textit{a priori}.

Finally, recall Tabs.~\ref{tab:U-wick-terms-phases} and \ref{tab:V-wick-terms-phases}, collecting the RWCs for the AF phase,
\[
\begin{aligned}
	&\hat H_U 
...
In this short section we recall the principal features of the AF phase established in conventional HM. The detailed derivation of all feature we here summarize is given in Sec.~\ref{appsec:antiferromagnetic-phase}. Before starting, let us define as in Eqns.~\eqref{appeq:af-hm-psi-def} and \eqref{appeq:af-hm-n-def} the AF and density fluctuations operators,
\begin{align}
...
\end{align}
which represent the two essential fermionic quadratic operators of the analytic MFT derivation. It is very useful to also note the following properties. Given the $\gamma$-wave form factors of Tab.~\ref{tab:x-wave-reciprocal-factors}, evidently both following equations hold
\[
\begin{aligned}
...
Within pure HM, AF phase is specified by the Ansatz \eqref{appeq:af-mean-field-ansatz},
\begin{equation}\label{eq:af-mean-field-ansatz}
	\langle \hat n_{\mathbf{r}\sigma} \rangle = n - (-1)^{x+y+\delta_{\sigma=\uparrow}} m
\end{equation}
which is explicitly breaking translational invariance in each spin sector, while preserving $\mathrm{U}^z(1)$ and $\mathrm{U}^c(1)$ symmetries. As reported in Tab.~\ref{tab:U-wick-terms-phases}, the only RWC comes in Hartree channel, since the local two-body attraction couples operators at opposite spin index. The hamiltonian $\hat H$ is reduced thereby to the form of Eq.~\eqref{appeq:af-hm-mft-h}
\begin{multline*}

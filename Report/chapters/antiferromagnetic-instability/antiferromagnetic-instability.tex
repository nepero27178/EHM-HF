...
\]
At half filling and low temperature, the system is well approximated by a completely filled electronic band $-\tilde{E}_\mathbf{k}$. This gives the aforementioned approximation of the free energy $f_\mathrm{MFT}^{(\mathrm{AF})}$.
...
\]
shown for a slightly more coarse-grained dataset in Fig.~\ref{fig:af-df-data-A128}, we see immediately how $\Delta f_\mathrm{MFT} \ge 0$ in all $(U,V)$ plane. The normal phase data of Sec.~\ref{sec:normal-numeric} having been simulated at $U=0$ due to their intrinsic $U$ independence. The AF phase is simulated in the same range $0 \le V \le 8$, but for $0 \le U \le 10$. The comparable normal phase dataset was just a repetition of the $U=0$ data at all $U$s.
%
\begin{figure}
	\centering

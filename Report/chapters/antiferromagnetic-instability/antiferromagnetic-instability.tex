...
\begin{enumerate}
	...
	\end{aligned}
	\]
	The term in square brackets is positive. Through trivial algebraic manipulations,
	\[
	\begin{aligned}
		e^{-\beta(E+\mu)}\left[e^{\beta(E-\mu)}+1\right]^2 &> e^{\beta(E-\mu)}\left[e^{-\beta(E+\mu)}+1\right]^2\\
		e^{-\beta(E+\mu)}\left[e^{2\beta(E-\mu)}+2e^{\beta(E-\mu)}+1\right] &> e^{\beta(E-\mu)} \left[e^{-2\beta(E+\mu)}+2e^{-\beta(E+\mu)}+1\right] \\
		e^{-\beta(E+\mu)} e^{\beta(E-\mu)} \left[e^{\beta(E-\mu)}+e^{-\beta(E-\mu)}\right] &> e^{\beta(E-\mu)} e^{-\beta(E+\mu)} \left[e^{-\beta(E+\mu)}+e^{\beta(E+\mu)}\right]
	\end{aligned}
	\]
	Simplifying recurring terms and reorganising the remaining ones we get:
	\[
	\begin{aligned}
		e^{-\beta\mu}e^{\beta E} + e^{\beta\mu}e^{-\beta E} &> e^{-\beta\mu}e^{-\beta E} + e^{\beta\mu}e^{\beta E} \\
	\end{aligned}
	\]
	{\color{tabred}[Redo this computation...]}
\end{enumerate}
...
\[
	U = 0,~0.05,~0.1,,~\dots,~4.0
	\qquad
	V = 0,~0.05,~0.1,,~\dots,~6.0
	\qquad
	\delta = 0.0
\]
We set $\delta=0.0$. {\color{tabred}As we anticipated in Sec.~\ref{sec:af-instability}, any finite doping implies immediate disruption of commensurate antiferromagnetism, a consideration that makes any simulation at $\delta >0$ of low physical significance.}
...
\begin{figure}
	...
	\caption{Plot of the self-consistent convergence values for $m$ (left panel) and isomagnetic curves computed on the same data (right panel). The solid lines in the right panel represent those paths in parametric space along which magnetization is constant. {\color{tabred}[Are $U=0.05$ data correct?]}}
	\label{fig:AF-m-data-C128}
\end{figure}

The convergence values for $m$ computed in the $UV$ plane are shown in the panels of Fig.~\ref{fig:AF-m-data-C128}, along with a level curves plot to identify isomagnetic curves (right panel). Tolerances on density and HFPs have been set to
\[
	\Delta n = 10^{-2}
	\qquad
...
\begin{figure}
	...
	\subfloat[$U$-dependency of $m$ varying $V$.]{
		...
		\label{subfig:AF-m-data-C256-pVar=V}
	}
	\subfloat[$V$-dependency of $m$ varying $U$.]{
		...
		\label{subfig:AF-m-data-C256-pVar=U}
	}
	\caption{Plot of the self-consistent convergence values for the HFPs $m$ and $u^{(s^*)}$ in the AF phase.}
\label{fig:AF-m-data-C256}
\end{figure}

The data shown in Fig.~\ref{fig:AF-m-data-C256} confirm clearly the aforementioned magnetization  ``boost'' given by $\hat H_V$. The effect is more enhanced at low values of $U$, and can be explained in terms of the competition measured by the ratio $V/U$.
...
\begin{figure}
	...
	\subfloat[$V$-dependency of $t^{(s^*)}$ in $UV$ plane.]{

...
\end{equation}
This is due to the fact that both $\tilde{\mathrm{Th}}_\mathbf{k}$ and $\tilde{E}_\mathbf{k}$ are positive-defined. An identical relation with inverted inequality holds for negative-defined function.
...
\begin{enumerate}
	...
	\end{equation}
	%
	\item Let $S_v \neq 0$ and $D_v \neq 0$. Then $A=C$. To prove this, consider the parabolic solutions of Eq.~\eqref{eq:saf-B2-4AC-relation},
	\[
		(B^2 - 4AC) V^2 - 2V (A+C) - 1 = 0
		\,\implies\,
		V_{1,2} = \frac{(A+C) \pm \sqrt{ (A+C)^2 + (B^2 - 4AC) }}{B^2 - 4AC}
	\]
	To identify a fixed point of the map, at least one of those points should satisfy Eq.~\eqref{eq:saf-B2-4AC-relation-int1}. Then it must be
	\[
	\begin{aligned}
		A+C \pm \sqrt{ (A+C)^2 + (B^2 - 4AC) } &= B+2C \\
		(A+C)^2 + (B^2 - 4AC) &= \left[ (B+2C) - (A+C) \right]^2 \\
		B^2 - 4AC &= (B+2C)^2 - 2(B+2C)(A+C) \\
		B^2 - 4AC &= B^2 + 4C^2 + 4BC - 2AB - 2BC - 4AC - 4C^2
	\end{aligned}
	\]

...
\begin{equation}\label{eq:af-V-fock-double-counting-int3}
	...
	\right]
\end{equation}
Interestingly enough, the new HFPs amplitudes contribute positively to the contraction energy. This fact is non-trivial and has consequences we will discuss in Sec.~\mref.
...
\subsection{Pure-symmetry gap and bands isotropy}

A very important point can be made about the relations between bands resymmetrization and the insurgence of an anisotropic gap. Consider the Bogoliubov band
\[
	\tilde{E}_\mathbf{k} = \sqrt{\tilde{\epsilon}_\mathbf{k}^2 + \big| \tilde{\Delta}_\mathbf{k} \big|^2}
\]
\todo

\subsection{Robust $\mathrm{C}_4$ bands shrinking extension}
...
\[
	u^{(s^*)} = - \frac{1}{L_x L_y} \sum_{\mathbf{k}\in\mathrm{BZ}} (\cos k_x + \cos k_y) \frac{\tilde{\epsilon}_\mathbf{k}}{\tilde{E}_\mathbf{k}} \tilde{\mathrm{Th}}[\beta,\tilde{\mu}]
\]

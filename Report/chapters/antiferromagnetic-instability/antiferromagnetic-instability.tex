...
\begin{align}
	\hat \psi_{\mathbf{k}+\bm{\pi}\sigma}^\dagger &= \hat \psi_{\mathbf{k}\sigma} \label{eq:af-(psi+pi)=psi}\\
	\varphi_{\mathbf{k}+\bm{\pi}}^{(\gamma)} &= -\varphi_\mathbf{k}^{(\gamma)} \label{eq:af-(phi+pi)=-phi}
\end{align}
...
In the AF phase, we have $\langle \hat n_{\mathbf{k}\sigma} \rangle \neq 0$ and $\langle \hat \psi_{\mathbf{k}\sigma} \rangle \neq 0$. This is proven by the following chains of equations:
\begin{align}
...
\end{align}
We used the Ansatz of Eq.~\eqref{eq:af-mean-field-ansatz}. Since $n \neq 0$, in the last sum $\langle \hat n_{\mathbf{k}\sigma} \rangle \neq 0$ is impossible for all $\mathbf{k}$s. Similarly, for the oscillatory part:
\begin{align}
...
\end{align}
where we defined the AF vector $\bm{\pi} \equiv (\pi,\pi)$. Thus, again using the Ansatz of Eq.~\eqref{eq:af-mean-field-ansatz},
\begin{align}
	...
	&= \frac{1}{L_x L_y} \sum_{\mathbf{r}\in\mathcal{L}} (-1)^{x+y} \langle \hat n_{\mathbf{r}\sigma} \rangle \nonumber \\
	&= \frac{1}{L_x L_y} \sum_{\mathbf{k} \in \mathrm{BZ}} \langle \hat \psi_{\mathbf{k}\sigma} \rangle \label{eq:af-m-EV}
\end{align}
where we used $1 = \left((-1)^{x+y}\right)^2$. If $m \neq 0$, in the last sum we cannot have $\langle \hat \psi_{\mathbf{k}\sigma} \rangle \neq 0$ for all $\mathbf{k}$s. We can restrict the sum to the MBZ and write it explicitly for both spins
\begin{align}
	m &= \frac{1}{L_xL_y} \sum_{\mathbf{k}\in\mathrm{MBZ}} \left[
	\hat \psi_{\mathbf{k}\uparrow} + \hat \psi_{\mathbf{k}\uparrow}^\dagger
	\right] \label{eq:af-m-EV-up}\\
	&= -\frac{1}{L_xL_y} \sum_{\mathbf{k}\in\mathrm{MBZ}} \left[
	\hat \psi_{\mathbf{k}\downarrow} + \hat \psi_{\mathbf{k}\downarrow}^\dagger
	\right] \label{eq:af-m-EV-down}
\end{align}
where we used the property of Eq.~\eqref{eq:af-(psi+pi)=psi}.
...
\]
We only need to analyse the Hartree contraction channel, as is reported in Tab.~\ref{tab:af-RWCs}. The HF decomposition and the Hartree sectors are given in Eq.~\eqref{eq:mft-HU}.
...
\subsection{Local Hartree channel in the AF phase: analytic derivation}\label{subsec:af-l-hartree-analytic}

The HF method, applied to a quartic operator, gives the decomposition of Eq.~\eqref{eq:mft-HU}. Decompose $\hat H_U$ in the Hartree channel:
\[
	...
	\right] \nonumber
\]
Note that in the AF phase, as is reported in Tab.~\ref{tab:HU-sel-rules}, the local repulsion decomposes just in the Hartree channel. Then no other term must be included in the above approximation of the quartic operator.
...
\end{equation}
with $E_{\mathrm{H}/U}^{(\mathrm{AF})}$ the HM Hartree contraction energy, discussed in Sec.~\ref{subsec:af-l-hartree-energy}. The first term of Eq.~\eqref{eq:af-hm-mft-h} gives a number operator,
\[
...
\begin{equation}\label{eq:af-l-mu}
	\mu \to \mu - nU
\end{equation}
We partially anticipated it in Tab.~\ref{tab:af-RWCs}. Another shift to $\mu$ will come from the non-local attraction. In the second term of Eq.~\eqref{eq:af-hm-mft-h}, apply Eq.~\eqref{eq:af-<psi>-not0-int}. It follows:
\[
...
Let us start from the operator of Expr.~\eqref{eq:af-V-wick-decomposition-operatorial}. By inserting the Ansatz of Eq.~\eqref{eq:af-mean-field-ansatz}, we get
\begin{multline*}
	\overbrace{
...
\end{enumerate}
To gain full knowledge on the map fixed point's shape, we should analyze similarly the behaviour of the self-consistency equations for $u^{(s^*)}$, $u^{(d)}$. This is done later in Sec.~\ref{subsec:af-bands-shrinking-C2}. For now, it suffices to note that -- limitedly to the sAF phase -- gap complexification is not present. This effect is not a consequence of HF derivation, which remains self-coherent and general; it is instead of the peculiar form of the non-linear map defined by the set of self-consistency equations.
...
\end{enumerate}
This ends this short section and shows that also for the aAF phase all gap complexifiers actually vanish. This is coherent with the constraint of Eq.~\eqref{eq:af-vgamma-circle}. As a consequence, there is no need to distinguish two AF phases,
\[
...
\end{enumerate}
These points end the proof: as anticipated, $f'$ is a monotonically descendent function of $\epsilon_\mathbf{k}$, a feature that allows us to re-use the entire derivation of Sec.~\ref{subsec:normal-bands-shrinking-C2}, and conclude that also for the AF phase
\[
	u^{(d)} = 0
\]
while $u^{(s^*)}$ remains constrained in the limits of Eq.~\eqref{eq:af-us*-constraint}. Starting from a rather complicated scenario, we were able to reduce all the AF HFPs down to the pair
\[
	m, u^{(s^*)}
\]
Perhaps not-so-excitingly, the real net effect of the non-local extension of the HM is, once again, the shrinking effect on bands. Now it is a matter of numeric analysis to assess if Mott localisation takes place.

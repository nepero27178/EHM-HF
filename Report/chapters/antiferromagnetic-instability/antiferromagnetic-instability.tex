...
\end{equation}
With this strategy, we get back to the application of HF methods to the full EHM, including the non-local attraction. \textbf{FROM HERE}
...
The gap correction is purely imaginary, compared to HM gap $\Delta_{\mathbf{k}\sigma} = (-1)^{\delta_{\sigma=\downarrow}} mU$. Since there is no other contribution to the $x$ pseudo-spin projections other than the pure HM ones, spin antisymmetry is preserved within such direction
\[
	\left\langle \hat s_{\mathbf{k}\sigma}^x \right\rangle = -\left\langle \hat s_{\mathbf{k}\overline{\sigma}}^x \right\rangle
\]
which is relevant during computation of the physical magnetization. From Eq.~\eqref{eq:af-hm-m-self-consistency-implicit}:
\[
\begin{aligned}
	m &= \frac{1}{2L_xL_y} \sum_{\mathbf{r}\in\mathcal{L}} (-1)^{x+y} \langle
	\hat n_{\mathbf{r}\uparrow} - \hat n_{\mathbf{r}\downarrow}
	\rangle \\

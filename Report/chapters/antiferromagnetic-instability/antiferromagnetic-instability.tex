...
The chosen Ansatz is coherent with the symmetry requirements of Tab.~\ref{tab:symmetry-groups}. Recall now Tabs.~\ref{tab:HU-sel-rules} and \ref{tab:HV-sel-rules}: the relevant Wick's contractions are, for each sector,
\[
\begin{aligned}
	&\hat H_U 
...
\section{Local Hartree effects: magnetisation}\label{sec:af-local-hartree-m}

Let us start from the local quartic interaction,
\[
	\hat H_U = U \sum_\mathbf{r} \hat c_{\mathbf{r}\uparrow}^\dagger \hat c_{\mathbf{r}\downarrow}^\dagger \hat c_{\mathbf{r}\downarrow} \hat c_{\mathbf{r}\uparrow}
\]
We only need to analyse the Hartree contraction channel, as is reported in Tab.~\ref{tab:af-RWCs}.

\subsection{Hartree channel in the normal phase: analytic derivation}\label{subsec:af-local-hartree-analytic}
...
\[
	...
	\right)
\]
Recall the Ansatz of Eq.~\eqref{eq:af-mean-field-ansatz}. From this rewriting we gain a definition and a justification for the Ansatz. On-site averaged density is:
...
\end{equation}
With this definitions, we have the decomposition:
\begin{equation}
	\hat H_U \simeq nU \sum_\mathbf{r} \left[
	\hat n_{\mathbf{r}\uparrow} + \hat n_{\mathbf{r}\downarrow}
	\right] - mU \sum_\mathbf{r} (-1)^{x+y} \left[
	\hat n_{\mathbf{r}\uparrow} - \hat n_{\mathbf{r}\downarrow}
	\right] - E_{\mathrm{H}/U}^{(\mathrm{AF})} \label{eq:af-hm-mft-h}
\end{equation}
with $E_{\mathrm{H}/U}^{(\mathrm{AF})}$ the HM Hartree contraction energy, discussed in Sec.~\ref{subsec:af-local-hartree-energy}. Note that the quantities $n$, $m$ in Eqns.~\eqref{eq:af-n-def}, \eqref{eq:af-m-def} are $\mathbf{r}$-independent. Then it must hold
\begin{align}
	n &= \frac{1}{L_x L_y} \sum_\mathbf{r} \frac{\ev{\hat n_{\mathbf{r}\uparrow}} + \ev{\hat n_{\mathbf{r}\downarrow}}}{2} \label{eq:af-hm-n-self-consistency-implicit}\\
	m &= \frac{1}{L_x L_y} \sum_\mathbf{r} (-1)^{x+y} \frac{\ev{\hat n_{\mathbf{r}\uparrow}} - \ev{\hat n_{\mathbf{r}\downarrow}}}{2} \label{eq:af-hm-m-self-consistency-implicit}
\end{align}
While from the first equation we gain an operational method to compute density, from the second we get a self-consistency equation for magnetisation.
...
\]
Chemical potential shift, anticipated in Tab.~\ref{tab:af-RWCs}, is present also in the EHM. In the second term, the phase factor can be absorbed through a momentum shift,
\begin{align}
...
\[
	...
	\right]
\]
Recall the definition of the MBZ given in Eq.~\eqref{eq:MBZ-def}. Using $2\bm{\pi}$ periodicity we rewrite
\begin{align}
...
\end{align}
Collecting everything, the local interaction reduces to
\[
...
\subsection{Hartree contraction energy}\label{subsec:af-local-hartree-energy}
...
\begin{align}
	...
	&= L_x L_y \times U \left(
	n^2 - m^2
	\right) \label{eq:af-hm-hartree-U-Ec}
\end{align}

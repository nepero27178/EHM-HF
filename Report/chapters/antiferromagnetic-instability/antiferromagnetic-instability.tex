...
\begin{table}
	...
	\begin{tabular}{r E l l}
		\textbf{Symmetry} & \multicolumn{2}{c}{\textbf{Self-consistency equation}} & \textbf{Graph} \\
		\midrule
		$s^*$-wave & $v^{(s^*)}$ & $\displaystyle \frac{1}{L_x L_y} \sum_{\mathbf{k}\in\mathrm{MBZ}} (\cos k_x + \cos k_y) \frac{\Im\lbrace\tilde{\Delta}_\mathbf{q}\rbrace}{\tilde{E}_\mathbf{q}} \tilde{\mathrm{Th}}_\mathbf{q}^{(-)}[\beta,\tilde{\mu}] $ & Fig.~\ref{subfig:s*-wave-correlator} \\[1.5em]
		$p_x$-wave & $v^{(p_x)}$ & $\displaystyle \frac{i\sqrt{2}}{L_x L_y} \sum_{\mathbf{k}\in\mathrm{MBZ}} \sin k_x \frac{\Im\lbrace\tilde{\Delta}_\mathbf{q}\rbrace}{\tilde{E}_\mathbf{q}} \tilde{\mathrm{Th}}_\mathbf{q}^{(-)}[\beta,\tilde{\mu}] $ & Fig.~\ref{subfig:px-wave-correlator} \\[1.5em]
		$p_y$-wave & $v^{(p_y)}$ & $\displaystyle \frac{i\sqrt{2}}{L_x L_y} \sum_{\mathbf{k}\in\mathrm{MBZ}} \sin k_y \frac{\Im\lbrace\tilde{\Delta}_\mathbf{q}\rbrace}{\tilde{E}_\mathbf{q}} \tilde{\mathrm{Th}}_\mathbf{q}^{(-)}[\beta,\tilde{\mu}] $ & Fig.~\ref{subfig:py-wave-correlator} \\[1.5em]
		$d_{x^2-y^2}$-wave & $v^{(d)}$ & $\displaystyle \frac{1}{L_x L_y} \sum_{\mathbf{k}\in\mathrm{MBZ}} (\cos k_x - \cos k_y) \frac{\Im\lbrace\tilde{\Delta}_\mathbf{q}\rbrace}{\tilde{E}_\mathbf{q}} \tilde{\mathrm{Th}}_\mathbf{q}^{(-)}[\beta,\tilde{\mu}] $ & Fig.~\ref{subfig:d-wave-correlator} \\
		\midrule
	\end{tabular}
...
For the sAF and aAf phases, the two possible five-entries HFPs real vectors are given respectively by:
\[
	\mathbf{v}_\mathrm{s} = \begin{bmatrix}
		m \\ u^{(s^*)} \\ u^{(d)} \\ v^{(s^*)} \\ v^{(d)}
	\end{bmatrix}
	\qquad
	\mathbf{v}_\mathrm{a} = \begin{bmatrix}
	m \\ u^{(s^*)} \\ u^{(d)} \\ -iv^{(p_x)} \\ -iv^{(p_y)}
	\end{bmatrix}
\]
\todo
...
In Fig.~\ref{fig:af-f-data} the solution free energy is plotted. In the Fig.~\ref{subfig:af-f-data-A256} it becomes evident how the energetically most stable state is realized at low $V$, high $U$. The opposite limit of weak local repulsion, strong non-local attraction (top-left corner of Fig.~\ref{subfig:af-f-data-C128}) instead increases $f_\mathrm{MFT}$ a lot, correctly reaching zero energy at $U=0$, $V>4.93t$ -- the critical value discussed in Sec.~\ref{subsec:normal-mott-numeric} and computed in Eq.~\eqref{eq:normal-Vc-half-filling}.

It is then evident how, even if the macroscopic magnetic properties are the same along one isomagnetic line of those plotted in Fig.~\ref{fig:af-m-data-C128}, right panel, and thus the realized state exhibits an identical value of $m$, much different is the energetic content of the state itself. While in the $U \gg 1$, fixed $V$ limit free energy gain comes from gap amplification -- which moves down the Bogoliubov bands $\tilde{E}_\mathbf{k}$ --, in the $V \gg 1$, fixed $U$ limit energies of the lower Bogoliubov band (full at low temperature) are shifted upwards via the Mott localization effect. \todo

%\begin{figure}
%	\centering
%	\subfloat[Heatmap of $s_\mathrm{MFT}$ in $\delta V$ plane.]{
%		\includegraphics[height=0.32\linewidth]{../Project/plots/refined/Mode=rs/Phase=AF/Setup=C[128]/Syms=/RB=S/Obj=phys/xVar=U_yVar=V/s_t=1.0_L=128_δ=0.0_β=100.0.pdf}
%		\label{subfig:af-s-data-C128}
%	}
%	\subfloat[$V$-dependency of $s_\mathrm{MFT}$ varying $\delta$.]{
%		\includegraphics[height=0.32\linewidth]{../Project/plots/refined/Mode=hs/Phase=AF/Setup=A[256]/Syms=/RB=S/Obj=phys/xVar=V_pVar=U/s_t=1.0_L=256_δ=0.0_β=100.0.pdf}
%		\label{subfig:af-s-data-A256}
%	}
%	\caption{Entropy density of the AF phase.}
%	\label{fig:af-s-data}
%\end{figure}

...
\subsection{More on AF Ansatz}\label{appsubsec:af-more}
...
%\end{equation}
%with $\left[\chi_{\hat{\mathbf{S}}^- \hat{\mathbf{S}}^+}(\bm{\pi},\omega)\right]$ the response function matrix obtained by considering a matrix response function for a two-sites unit cell. The above equation is to be interpreted as a matrix equation {\color{tabred}[Explain better..]}. Considering the eigenvalue of largest amplitude of the matrix $\lambda_\delta$ at filling $\delta$, the one most prone to instabilities, it is found that
%\[
%\begin{aligned}
...
{\color{tabred}Recall the result of Eq.~\eqref{HU-Fourier} which gave the Fourier-transformed version of the local repulsion, and perform a Cooper class Wick's contraction on it,
\begin{multline}

...
We work in a commensurate AF phase ordering, which is, we explicitly break symmetry by realizing a SDW phase with spin density unevenly distributed on sites with periodicity of two sites, each direction. Recalling the discussion of Sec.~\ref{sec:ehm-symmetries}, AF long-range ordering we deal with here is described by the composite SSB
\[
\begin{aligned}
...
\end{aligned}
\]
Recalling Tab.~\ref{tab:U-wick-terms-phases}, for such a physical phase the only Wick contraction channel available is the Hartree one. Let us proceed in such sense. As in Eqns.~\eqref{eq:af-ehm-n-def}, \eqref{eq:af-ehm-psi-def}, it is useful to define the density and AF fluctuation operators:
\begin{align}
	\hat n_{\mathbf{k}\sigma} &\equiv \hat c_{\mathbf{k}\sigma}^\dagger \hat c_{\mathbf{k}\sigma} \label{appeq:af-hm-n-def} \\
	\hat \psi_{\mathbf{k}\sigma} &\equiv \hat c_{\mathbf{k}+\bm{\pi}\sigma}^\dagger \hat c_{\mathbf{k}\sigma} \label{appeq:af-hm-psi-def}
\end{align}
...
\]
an effect that is present also in the EHM. The kinetic part of the hamiltonian transforms as in Eq.~\eqref{eq:Ht-Fourier},
\[
	-t \sum_{\langle \mathbf{r}\mathbf{r}' \rangle} \sum_\sigma \left[
	\hat c_{\mathbf{r}\sigma}^\dagger \hat c_{\mathbf{r}'\sigma} + \hc
	\right] \stackrel{\mathcal{F}}{=} \sum_{\mathbf{k} \in \mathrm{BZ}} \sum_\sigma \epsilon_\mathbf{k} \hat c_{\mathbf{k}\sigma}^\dagger \hat c_{\mathbf{k}\sigma}
\]
being $\epsilon_\mathbf{k}$ the bare bands of Eq.~\eqref{eq:bare-bands}.
...
\end{align}
and reduce Eq.~\eqref{appeq:af-hm-mft-h} to its reciprocal form
\[
	\hat H \simeq \sum_{\mathbf{k}\in\mathrm{MBZ}} \sum_\sigma \epsilon_\mathbf{k} \left[
...
\[
\begin{aligned}
	\hat n_{\mathbf{r}\uparrow} \hat n_{\mathbf{r}\downarrow} - \mathrm{d.c.} &=
	\hat n_{\mathbf{r}\uparrow} \ev{\hat n_{\mathbf{r}\downarrow}} + \ev{\hat n_{\mathbf{r}\uparrow}} \hat n_{\mathbf{r}\downarrow} \\
	&= 
	\hat n_{\mathbf{r}\uparrow} \frac{\ev{\hat n_{\mathbf{r}\downarrow}} + \ev{\hat n_{\mathbf{r}\uparrow}}}{2} + \hat n_{\mathbf{r}\uparrow} \frac{\ev{\hat n_{\mathbf{r}\downarrow}} - \ev{\hat n_{\mathbf{r}\uparrow}}}{2} 
	+
	\frac{\ev{\hat n_{\mathbf{r}\uparrow}} + \ev{\hat n_{\mathbf{r}\uparrow}}}{2} \hat n_{\mathbf{r}\downarrow} + \frac{\ev{\hat n_{\mathbf{r}\uparrow}} - \ev{\hat n_{\mathbf{r}\uparrow}}}{2} \hat n_{\mathbf{r}\downarrow} \\
...
\begin{align}
	n &= \frac{1}{L_x L_y} \sum_\mathbf{r} \frac{\ev{\hat n_{\mathbf{r}\uparrow}} + \ev{\hat n_{\mathbf{r}\downarrow}}}{2} \label{appeq:af-hm-n-self-consistency-implicit}\\
	m &= \frac{1}{L_x L_y} \sum_\mathbf{r} (-1)^{x+y} \frac{\ev{\hat n_{\mathbf{r}\uparrow}} - \ev{\hat n_{\mathbf{r}\downarrow}}}{2} \label{appeq:af-hm-m-self-consistency-implicit}
\end{align}
...
We now map the reciprocal version of the HM onto a simpler problem. Throughout this section we use the notation $\epsilon_\mathbf{k} \to \epsilon_{\mathbf{k}\sigma}$ with identical meaning. Let the Nambu spinors:
\[
...
\begin{equation}\label{appeq:af-hk}
...
Finally, using Eq.~\eqref{appeq:af-hm-gamma-ops-transform}, we check that correctly the ground-state describes a perfect degenerate $\gamma^{(-)}$ quasiparticles Fermi gas,
...
\[
	...
	\right\rangle
\]
due to the $W_{\mathbf{k}\sigma}$ unitarity properties, the total number of electrons is given by (also recalling Eq.~\eqref{appeq:af-hm-n-self-consistency-implicit})
\begin{align}
...
\[
	\frac{1}{2L_xL_y} \sum_\mathbf{r} (-1)^{x+y} \hat n_{\mathbf{r}\sigma} = \frac{1}{2L_xL_y} \sum_{\mathbf{k}\in\mathrm{MBZ}} \left[
		\hat \psi_{\mathbf{k}\sigma} + \hat \psi_{\mathbf{k}\sigma}^\dagger
	\right]
\]
thus employing Eq.~\eqref{appeq:af-hm-m-self-consistency-implicit}
\[
\begin{aligned}
...
\begin{align}
	...
	&= \frac{1}{L_x L_y} \sum_{\mathbf{k} \in \mathrm{MBZ}} \frac{\Delta}{E_\mathbf{k}} \mathrm{Th}_\mathbf{k}^{(-)}[\beta,\mu] \label{appeq:af-hm-m-self-consistency}
\end{align}
...
Consider specifically the half-filled situation and multiply Eq.~\eqref{appeq:af-hm-m-self-consistency} on both sides by $U$. Since due to symmetry $\mu=0$, we have the finite-temperature self-consistent equation
\[
...
\[
	\frac{1}{U} \simeq \int_\mathrm{MBZ} \frac{\mathrm{d}\mathbf{k}}{(2\pi)^2} \frac{1}{\sqrt{\epsilon_{\mathbf{k}}^2 + \Delta^2}} \tanh(\frac{\beta}{2} \sqrt{\epsilon_{\mathbf{k}}^2 + \Delta^2})
\]
...
\[
	\frac{1}{U} \simeq  \int_\mathrm{bands}  \frac{\mathrm{d}\epsilon \rho(\epsilon)}{\sqrt{\epsilon^2 + \Delta^2}}
\]
\todo

\subsection{Instability of the commensurate AF solution and atypical metallic bands}\label{appsubsec:af-instability}

A key point to be discussed is the known instability of the AF HF solution of the EHM. As is thoroughly discussed in \cite{singh1990collective}, the solution we presented above is only stable at half-filling, becoming hardly unstable at infinitesimal doping. This is due to the commensurate nature of the established AF phase: we impose SSB to take place at wavevector $\bm{\pi}$, shrinking the BZ to exactly half its size and giving back on the newly defined MBZ two bands $\pm E_\mathbf{k}$. This procedure actually works well for the half-filled case: magnetization essentially depends on the scattering rate between points on the FS connected by a $\bm{\pi}$ vector. The half filled case, taking advantage of the natural nesting property of the bare bands,
\[
...
\end{equation}
being $\chi$ the free transverse response. The pseudo-spin hereby considered is the one of Eq.~\eqref{af-hm-spin-h}. As is shown in \cite{singh1990collective}, for the commensurate AF SSB at wavevector $\bm{\pi}$ the condition for the presence of a stable gapless mode reduces to a $2\times 2$ matrix equation,
...
For the sake of completeness, we decided to run our algorithm also at finite doping, knowing from the previous discussion of Sec.~\ref{appsubsec:af-instability} that the results at $\delta > 0$ are unstable. The algorithm sketched hereby was ran with the following setup:
...
\begin{figure}
	...
	\subfloat[Local repulsion $U/t$ depencence.]{
		\includegraphics[height=0.32\linewidth]{../Project/plots/raw/Mode=hs/Phase=AF/Setup=D[256]-a/Syms=/RB=S/Obj=HFPs/xVar=U_pVar=δ/m_t=1.0_V=0.0_L=256_β=100.0.pdf}
		\label{appsubfig:mU-beta=100}
	}
	\subfloat[Doping $\delta = n-0.5$ dependence.]{
		\includegraphics[height=0.32\linewidth]{../Project/plots/raw/Mode=hs/Phase=AF/Setup=D[256]-a/Syms=/RB=S/Obj=HFPs/xVar=δ_pVar=U/m_t=1.0_V=0.0_L=256_β=100.0.pdf}
		\label{appsubfig:dU-beta=100}
	}	
	\caption{Plots of the zero-temperature AF instability order parameter $m$ as a function of both the local repulsion $U/t$ (Fig.~\ref{appsubfig:mU-beta=100}) and the doping $\delta = n-1/2$ (Fig.~\ref{appsubfig:dU-beta=100}). {\color{tabred}[Insert new more refined data.]}}
...
Recall the result of Eq.~\eqref{HU-Fourier} which gave the Fourier-transformed version of the local repulsion, and perform a Cooper class Wick's contraction on it,
\begin{multline}
...
\end{multline}
We are not breaking translational invariance, thus only Cooper fluctuations with net zero total momentum are allowed. This means only $\mathbf{K}=\mathbf{0}$ contributes. Define as in Eq.~\eqref{eq:sc-ehm-phi-def} the pairing operator 
\begin{equation}\label{appeq:sc-hm-phi-def}
	\hat \phi_\mathbf{k} \equiv \hat c_{-\mathbf{k}\downarrow} \hat c_{\mathbf{k} \uparrow}
\end{equation}
and define $w_\mathbf{q}$ as in Eq.~\eqref{eq:sc-wq-def},
\[
...
\end{equation}
Let $\hat n_{\mathbf{k}\sigma}$ as in Eq.~\eqref{appeq:af-hm-n-def}. Eq.~\eqref{appeq:sc-hm-U-int-1} reduces to
...
\begin{align}
	E_{\mathrm{C}/U}^{(\mathrm{SC})} &\equiv \frac{U}{L_x L_y} \sum_{\mathbf{q}\mathbf{q}'} \big\langle
		\hat \phi_\mathbf{q}^\dagger
	\big\rangle \big\langle
		\hat \phi_{\mathbf{q}'}
	\big\rangle \nonumber \\
	&= L_x L_y \times U \big| w^{(s)} \big|^2 \label{appeq:sc-hm-hartree-C-Ec}
\end{align}
Let now
\[
	\Delta^{(s)} \equiv -U w^{(s)}
\]
We finally reduce $\hat H_U$ to
...
\begin{equation}\label{appeq:sc-hm-bogoliubov-h}
	\hat H - \mu \hat N \simeq \sum_{\mathbf{k} \in \mathrm{BZ}} \sum_\sigma \hat \Phi_\mathbf{k}^\dagger h_\mathbf{k} \hat \Phi_\mathbf{k} + E_\mathrm{a}^{(\mathrm{SC})}- \left[
		E_{\mathrm{H}/U}^{(\mathrm{SC})} + E_{\mathrm{C}/U}^{(\mathrm{SC})}
	\right]
\end{equation}
...
\begin{equation}\label{appeq:af-sc-spin-h}
	\hat H -\mu \hat N \simeq \sum_{\mathbf{k} \in \mathrm{BZ}} \mathbf{b}_\mathbf{k} \cdot \hat{\mathbf{s}}_\mathbf{k} + E_\mathrm{a}^{(\mathrm{SC})} - \left[
		E_{\mathrm{H}/U}^{(\mathrm{SC})} + E_{\mathrm{C}/U}^{(\mathrm{SC})}
	\right]
	\qq{where}
...
\]
Hence, we have
\begin{equation}\label{appeq:sc-hm-gs}
	\ket{\Psi} = \bigotimes_{\mathbf{k}\in\mathrm{BZ}} \left[
		\cos \theta_\mathbf{k} + e^{2i\zeta_\mathbf{k}} \sin \theta_\mathbf{k} \hat \phi_\mathbf{k}^\dagger
...
and the EVs:
\begin{align}
	\ev{\hat s_\mathbf{k}^x} &= \frac{\Re\lbrace\Delta_\mathbf{k}\rbrace}{E_\mathbf{k}} \mathrm{Th}_\mathbf{k}^{(-)}[\beta] \label{appeq:sc-hm-sx}\\
	\ev{\hat s_\mathbf{k}^y} &= \frac{\Im\lbrace\Delta_\mathbf{k}\rbrace}{E_\mathbf{k}}  \mathrm{Th}_\mathbf{k}^{(-)}[\beta] \label{appeq:sc-hm-sy}\\
	\ev{\hat s_\mathbf{k}^z} &=  -\frac{\xi_\mathbf{k}}{E_\mathbf{k}}  \mathrm{Th}_\mathbf{k}^{(-)}[\beta] \label{appeq:sc-hm-sz}
\end{align}
...
\begin{align}
	...
	&= \frac{1}{2 L_x L_y} \sum_{\mathbf{k}\in\mathrm{BZ}} \frac{\Delta_\mathbf{k}}{E_\mathbf{k}} \mathrm{Th}_\mathbf{k}^{(-)}[\beta] \label{appeq:sc-hm-ws-self-consistency-2}
\end{align}
...
\]
Since evidently $U_\mathrm{c}[\beta] > 0$ by definition (the thermal factor is negative by construction), the above equation can be solved in $\mathbb{C}$ only by $w^{(s)}=0$. This implication terminates the argument: superconductivity cannot establish self-consistently at this level in the HM.

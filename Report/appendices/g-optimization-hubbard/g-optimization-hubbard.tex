\chapter{Mixing optimization in HM model}\label{appendix:sc-g-opt}

This short appendix is meant to give a practical example on a topic we often referenced to in text: how optimization of $g$, the mixing parameter defined in Sec.~\ref{subsec:reject-mix-iterate} and given operatively in  Eq.~\eqref{eq:mix} as
\[
	\mathbf{v}_{i+1} \equiv g \mathrm{A} \left( \mathbf{v}_{i} \right) + (1-g) \mathbf{v}_{i}
\]
can be relevant in order to ensure convergence of an iterative HF scheme as presented in Sec.~\ref{subsec:hf-sketch}.

\section{Map analysis for $s$-wave superconductivity in HM}

As we thoroughly discussed in Sec.~\ref{appsec:superconducting-phase}, conventional repulsive HM cannot exhibit BCS $s$-wave superconductivity within a MFT general scheme. This is proven trivially at the end of Sec.~\ref{appsubsec:sc-hm-impossible}, where we end up withe the self-consistency equation

...
This short appendix is meant to give a practical example on a topic we often referenced to in text: how optimisation of $g$, the mixing parameter defined in Sec.~\ref{subsec:reject-mix-iterate} and given operatively in  Eq.~\eqref{eq:mix} through
\[
...
As we discussed in Sec.~\ref{subsec:sc-hm}, conventional repulsive HM cannot exhibit BCS $s$-wave superconductivity within a MFT general scheme. This is proven at the end of Sec.~\ref{subsec:sc-hm}, where we end up with the self-consistency equation
\begin{equation}\label{appeq:sc-self-consistency}
	w^{(s)} = -\frac{U}{U_\mathrm{c}[\beta]} w^{(s)} \, ,
\end{equation}
where $U_\mathrm{c}[\beta]$ is defined as in Eq.~\eqref{eq:sc-hm-Uc},
\begin{equation}\label{appeq:sc-hm-Uc}
	U_\mathrm{c}[\beta] \equiv \left[
	\frac{1}{2 L_x L_y} \sum_{\mathbf{k}\in\mathrm{BZ}} \frac{\mathrm{Th}_\mathbf{k}^{(-)}[\beta]}{E_\mathbf{k}}
	\right]^{-1} \, .
\end{equation}
Since by construction $U_\mathrm{c}[\beta] > 0$ when $U>0$, Eq.~\eqref{appeq:sc-self-consistency} can only be solved by $w^{(s)}=0$. Now, our aim is the following: given any initialiser $w^{(s)}_0$, our iterative HF algorithm should give
\[
	w^{(s)}_0
	...
	\sim 0 \, .
\]
We ask: is this algorithmic evolution possible at any model parametrisation?

\subsection{Constraints for algorithmic evolution}
...
\begin{equation}\label{appeq:sc-map-g}
...
\end{aligned}
\]
The subscript indicate a ``soft boundary'' (first) and an ``hard boundary'' (second). If $w_i>0$:
\begin{enumerate}
	\item It always holds $w_{i+1} \le w_i$. If instead $w_{i+1}>w_i$, from Eq.~\eqref{appeq:sc-map-g} we have
	\[
	\begin{aligned}
	...
	\end{aligned}
	\]
	which is impossible because a sum of negative terms cannot be positive.
	%
	...
	\]
	%
	\item For $U > U_\mathrm{h}$, we have $w_{i+1} < -w_i$. This is proven by inverting inequalities in last point.
\end{enumerate}

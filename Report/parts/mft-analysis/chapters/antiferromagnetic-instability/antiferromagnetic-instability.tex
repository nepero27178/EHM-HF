...
\[
	\langle
		\hat \psi_{\mathbf{k}\sigma} \equiv \hat c_{\mathbf{k}\sigma}^\dagger \hat c_{\mathbf{k}+\bm{\pi}\sigma}
	\rangle
\]
...
\begin{align}
	\hat n_{\mathbf{k}\sigma} &\equiv \hat c_{\mathbf{k}\sigma}^\dagger \hat c_{\mathbf{k}\sigma} \label{eq:af-hm-n-def} \\
	\hat \psi_{\mathbf{k}\sigma} &\equiv \hat c_{\mathbf{k}\sigma}^\dagger \hat c_{\mathbf{k}+\bm{\pi}\sigma} \label{eq:af-hm-psi-def}
\end{align}
which represent the two essential fermionic quadratic operators of the analytic MFT derivation. Within the pure HM, the AF phase is specified by the Ansatz \eqref{appeq:af-mean-field-ansatz} which is explicitly breaking translational invariance in each spin sector, while preserving $\mathrm{U}^z(1)$ and $\mathrm{U}^c(1)$ symmetries. As reported in Tab.~\ref{tab:U-wick-terms-phases}, the only RWC comes in Hartree channel, since the local two-body interaction couples operators at opposite spin index. The hamiltonian $\hat H$ is reduced thereby to the form of Eq.~\eqref{appeq:hubbard-mean-field-hamiltonian}
\begin{multline*}
...
\end{multline*}
with $E_{\mathrm{H}/U}^{(\mathrm{AF})}$ the contraction energy resulting from double counting terms, given explicitly in Eq.~\eqref{appeq:af-hartree-V-contraction-energy}. Ignore this energy shift for now. In reciprocal space, the hamiltonian decomposes as in Eq.~\eqref{appeq:hubbard-bogoliubov-hamiltonian},
\[
...
\begin{table}
	...
	\begin{tabular}{c c c}
		...
		}$ & \eqref{appeq:hm-af-sx} \\[0.7em]
		$i\ev{\hat s_{\mathbf{k}\sigma}^y}$ & $\ev{
			\hat \psi_{\mathbf{k}\sigma}
			- \hat \psi_{\mathbf{k}\sigma}^\dagger
		}$ & \eqref{appeq:hm-af-sy} \\[0.7em]
		$\ev{\hat s_{\mathbf{k}\sigma}^z}$ & $\ev{
			\hat n_{\mathbf{k}\sigma}
			+ \hat n_{\mathbf{k}+\bm{\pi}\sigma}
		}$ & \eqref{appeq:hm-af-sz} \\
		\midrule
	\end{tabular}
...
\end{align}
while in the tilted frame, the $z$ component of the pseudospin is just a thermal superposition of the ``up'' and ``down'' states
\begin{equation}\label{eq:af-rot-z-expectation}
	...
	\right]
\end{equation}
These equations are identical to the ones for the pure HM, Eqns.~\eqref{appeq:hm-af-sx}, \eqref{appeq:hm-af-sy} and \eqref{appeq:hm-af-sz}. Let us define the (renormalized) thermal factors as in Eq.~\eqref{appeq:af-thermal-factors-def},
...
Consider the s.s. and o.s. (density) terms of Expr.~\eqref{eq:af-hartree-renormalization-intermediate},
...
The $\mathrm{s.s.}$ and $\mathrm{o.s.}$ (magnetization) terms of Expr.~\eqref{eq:af-hartree-renormalization-intermediate} perfectly cancel out:
\[
	mV \sum_{\ev{\mathbf{r}\mathbf{r}'}} \sum_\sigma (-1)^{x+y+\delta_{\sigma=\uparrow}} \left[
		\left(
			\hat n_{\mathbf{r}\sigma} - \hat n_{\mathbf{r}'\sigma}
...
%\end{aligned}
%\]
%Thus, Eq.~\eqref{eq:af-hartree-renormalization-intermediate-2} reduces to:
%\begin{multline}
%	mV \sum_{\ev{\mathbf{r}\mathbf{r}'}} \sum_\sigma (-1)^{x+y+\delta_{\sigma=\uparrow}} \left[
%	\hat n_{\mathbf{r}\sigma} - \hat n_{\mathbf{r}'\sigma}
%	\right] + mV \sum_{\ev{\mathbf{r}\mathbf{r}'}} \sum_\sigma (-1)^{x+y+\delta_{\sigma=\uparrow}} \left[
%	\hat n_{\mathbf{r}\sigma} + \hat n_{\mathbf{r}'\overline{\sigma}}
%	\right] \\
%	= 2zmV \sum_{\mathbf{k} \in \mathrm{BZ}} \sum_\sigma (-1)^{\delta_{\sigma=\uparrow}} \hat c_{\mathbf{k}\sigma}^\dagger \hat c_{\mathbf{k}+\bm{\pi}\sigma} \label{eq:af-hartree-renormalization-intermediate-3}
%\end{multline}
...
\section{Non-local Fock effects: gap and bands renormalization}\label{sec:af-fock-bands-renormalization}
...
\end{multline}
where $E_{\mathrm{F}/V}^{(\mathrm{AF})}$ is defined as in Eq.~\eqref{eq:reciprocal-space-non-local-interaction-fock-double-couting}, with the expectation value being computed on the AF (thermal) ground-state. As before, we discuss the operatorial part and the scalar part separately.
...
\end{align}
As we will discuss later in Sec.~\ref{subsec:ehm-af-flavours}, not all these HFPs can coexist simultaneously. For now, let us move to the free energy density computation.
...
Let us now get back to $E_{\mathrm{F}/V}^{(\mathrm{AF})}$, the double counting energy of Eq.~\eqref{eq:af-V-fock-int1}. As said before, its definition is the same as for the normal phase, Eq.~\eqref{eq:reciprocal-space-non-local-interaction-fock-double-couting}, with the averaging operation being performed on the AF ground state:
...
\subsection{The relative phase in the zero temperature ground state}

\todo

\subsection{AF phase flavours and spin degeneracy}\label{subsec:ehm-af-flavours}

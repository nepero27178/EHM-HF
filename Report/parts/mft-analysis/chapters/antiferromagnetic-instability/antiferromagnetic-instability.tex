...
\subsection{Symmetry considerations for the AF phase}\label{subsec:af-symmetry-considerations}

The antiferromagnetic phase transition is simply described in reciprocal space by the order parameter 
\[
	\langle
		\hat c_{\mathbf{k}\sigma}^\dagger \hat c_{\mathbf{k}+\bm{\pi}\sigma}
	\rangle
\]
When such an EV acquires a non-zero $s$-wave value, commensurate finite magnetization is established. \todo
...
\section{Fock effects: bands renormalization}\label{subsec:af-fock-bands-renormalization}

We explored thoroughly the Hartree contractions effects on the MFT hamiltonian. Let us now move to the effects originated by Fock contractions. We can use the derivation of Sec.~\ref{subsec:fock-hopping-renormalization-normal} up to Eq.~\eqref{eq:normal-V-fock-int2},
\begin{multline}
	V \sum_{\ev{ij}} \sum_\sigma \left[
		\langle
...
\subsection{Full MFT hamiltonian and gap function}
...
\subsection{Symmetry considerations and AF phase flavour}\label{subsec:ehm-af-flavours}

...
\section{Hartree effects: chemical potential renormalization}\label{subsec:af-hartree-mu}
...
In order to proceed from Eq.~\eqref{eq:af-V-fock-int1}, it is necessary to understand how the AF phase is realized in reciprocal space. For the conventional Hubbard model, as is exposed in App.~\ref{appendix:mean-field-hubbard}, to impose an AF Ansatz of the form
\[
	\langle \hat n_{\mathbf{r}\sigma} \rangle = n - (-1)^{x+y+\delta_{\sigma=\uparrow}} m
\]
...
\[
	...
	\left(\hat \gamma_{\mathbf{k}\sigma}^{(-)}\right)^\dagger f(-\tilde{E}_\mathbf{k};\beta,\tilde{\mu}) + \left(\hat \gamma_{\mathbf{k}\sigma}^{(+)}\right)^\dagger f(\tilde{E}_\mathbf{k};\beta,\tilde{\mu})
	\right] \ket{\Omega}
\]
...
\]
The first case gives us the normal fermionic EV, $\langle \hat c_{\mathbf{k}\sigma}^\dagger \hat c_{\mathbf{k}\sigma} \rangle$, the second the staggered AF order parameter, $\langle \hat c_{\mathbf{k}\sigma}^\dagger \hat c_{\mathbf{k}+\bm{\pi}\sigma} \rangle$\footnote{
	All of this is equivalent to saying that we are explicitly breaking symmetry using the order parameter $\langle \hat c_{\mathbf{k}\sigma}^\dagger \hat c_{\mathbf{k}+\bm{\pi}\sigma} \rangle$, and \textit{thus} the state is realized as described in text.
...
\]
The $s^*$-wave component is particularly handy, because it can interpreted as a rigid hopping shift,
\[
	t \to \tilde{t}_\sigma = t - \frac{1}{2} u_\sigma^{(s^*)} V
\]
As for the normal phase, this RMP is controlled linearly by $V$ and can lead to a relevant reduction of the bare bands diffusive character.
...
\end{align}
The general HF scheme, described in Sec.~\mref, is iterative and aims to use these equations iteratively to find a stable solution for these parameters.
...
\begin{equation}\label{eq:af-vqsigma-def}
	...
	\rangle
\end{equation}
(which is just the AF order parameter) and its $\gamma$-wave component, with $\gamma\in\lbrace s^*,d,p_x,p_y\rbrace$:
\begin{align}
...
\begin{multline}
	...
	\left[
		...
		\hat c_{\mathbf{q}\sigma}^\dagger
		\hat c_{\mathbf{q}+\bm{\pi}\sigma} + v^{(\gamma)}_\sigma \varphi_{\mathbf{q}+\bm{\pi}}^{(\gamma)*}
		\hat c_{\mathbf{q}+\bm{\pi}\sigma}^\dagger
		\hat c_{\mathbf{q}\sigma}
	\right]
...
\begin{multline}
	...
	\left[
		iv^{(\gamma)}_\sigma \varphi_\mathbf{q}^{(\gamma)*}
		\hat c_{\mathbf{q}\sigma}^\dagger
		\hat c_{\mathbf{q}+\bm{\pi}\sigma} + \hc
	\right]
	...
	\Big[
		...
		\underbrace{
			\langle
				...
				\hat c_{\mathbf{q}+\bm{\pi} \sigma}
			\rangle \langle
				\hat c_{\mathbf{q}'\sigma}^\dagger \hat c_{\mathbf{q}'+\bm{\pi} \sigma}
			\rangle
		}_\text{Off-diagonal terms}
	\Big]
\]
...
\begin{multline*}
	\hat H \simeq \sum_\sigma \sum_{\mathbf{q}\in\mathrm{MBZ}} \tilde{\epsilon}_{\mathbf{k}\sigma} \left[
		\hat c_{\mathbf{k}\sigma}^\dagger \hat c_{\mathbf{k}\sigma} - \hat c_{\mathbf{k}+\bm{\pi}\sigma}^\dagger \hat c_{\mathbf{k}+\bm{\pi}\sigma}
	\right] \\
	- \sum_\sigma \sum_{\mathbf{q}\in\mathrm{MBZ}} \left[
		\tilde{\Delta}_{\mathbf{k}\sigma}
		\hat c_{\mathbf{k}\sigma}^\dagger \hat c_{\mathbf{k}+\bm{\pi}\sigma} + \tilde{\Delta}_{\mathbf{k}\sigma}^* \hat c_{\mathbf{k}+\bm{\pi}\sigma}^\dagger \hat c_{\mathbf{k}\sigma}
	\right] - \left[
...
\begin{aligned}
	...
	\hat n_{\mathbf{r}\uparrow} - \hat n_{\mathbf{r}\downarrow}
	\rangle \\
	&= \frac{1}{2L_xL_y} \sum_{\mathbf{k} \in \mathrm{BZ}} \langle
	\hat c_{\mathbf{k}\uparrow}^\dagger \hat c_{\mathbf{k}+\bm{\pi}\uparrow} - \hat c_{\mathbf{k}\downarrow}^\dagger \hat c_{\mathbf{k}+\bm{\pi}\downarrow}
	\rangle \\
...
\end{aligned}
\]
where we procedeed as in Sec.~\ref{appsubsec:af-self-consistency-gap-equation}.

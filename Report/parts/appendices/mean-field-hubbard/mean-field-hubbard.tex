...
\begin{equation}\label{appeq:hubbard-bogoliubov-hamiltonian-pseudofields}
	...
	\end{bmatrix}
\end{equation}
The hamiltonian represents a system of spins subject to local magnetic fields, each tilted by an angle $\tan(2\theta_{\mathbf{k}\sigma}) = \Delta_{\mathbf{k}\sigma}/\epsilon_{\mathbf{k}\sigma}$, as sketched in Fig.~\ref{appfig:pseudo-magnetic-field}. Evidently the following relations hold:
\begin{align}
	...
	\hat s_{\mathbf{k}\sigma}^z &= \hat n_{\mathbf{k}\sigma} - \hat n_{\mathbf{k}\sigma}^\dagger \label{appeq:af-hm-z-psi} 
\end{align}
Let also:
\begin{align}
	\hat \Id_{\mathbf{k}\sigma} &\equiv \hat \Psi_{\mathbf{k}\sigma}^\dagger \hat \Psi_{\mathbf{k}\sigma} \nonumber \\
...
\begin{equation}\label{appeq:af-full-h-diagonalization}
	...
	h_{\mathbf{k}\sigma}-\mu \hat \Id_{\mathbf{k}\sigma}
	\right] W_{\mathbf{k}\sigma}^\dagger = d_{\mathbf{k}\sigma}-\mu \hat \Id_{\mathbf{k}\sigma}
\end{equation}
...
\subsection{Explicit zero-temperature ground state}

The AF ground state can be expressed easily. Let $\ket{\Uparrow_{\mathbf{k}\sigma}}$ and $\ket{\Downarrow_{\mathbf{k}\sigma}}$ be respectively the ``up'' and ``down'' $\mathbf{k}$-th pseudospin states. Let $\ket{\overline{\Downarrow}_{\mathbf{k}\sigma}}$ be the ``down'' state in the tilted frame. At $\beta \to +\infty$, the ground state is given by
...
\begin{align}
	...
	&= \cos \theta_{\mathbf{k}\sigma} \ket{\Downarrow_{\mathbf{k}\sigma}} - \sin\theta_{\mathbf{k}\sigma} \ket{\Uparrow_{\mathbf{k}\sigma}} \label{appeq:af-tilted-down}
\end{align}
having we used
\[
...
It follows that composing Eqns.~\eqref{appeq:af-hm-gs-1} and \eqref{appeq:af-tilted-down} the ground state $\ket{\Psi}$ can be written as:
\begin{equation}\label{appeq:af-hm-gs-2}
	\ket{\Psi} = \bigotimes_\sigma \bigotimes_{\mathbf{k}\in\mathrm{MBZ}} \left[
		\cos \theta_{\mathbf{k}\sigma} \hat c_{\mathbf{k}+\bm{\pi}\sigma}^\dagger + e^{2i\zeta_{\mathbf{k}\sigma}} \sin\theta_{\mathbf{k}\sigma} \hat c_{\mathbf{k}\sigma}^\dagger
	\right] \ket{\Omega_{\mathbf{k}\sigma}}
\end{equation}
having we absorbed in the relative phase $e^{2i\zeta_{\mathbf{k}\sigma}}$ the two states phases differences as well as the original sign difference of Eq.~\eqref{appeq:af-tilted-down}. Now, inserting the explicit form of the ground state from equation above:
\[
\begin{aligned}
...
\begin{align}
	...
	&= -\frac{1}{2L_x L_y} \sum_{\mathbf{k} \in \mathrm{MBZ}} \frac{\Delta}{2E_\mathbf{k}} \mathrm{Th}_\mathbf{k}^{(-)}[\beta,\mu] \label{appeq:hm-af-m-self-consistency}
\end{align}
...
Consider specifically the half-filled situation and multiply Eq.~\eqref{appeq:hm-af-Delta-self-consistency} on both sides by $U$. Since due to symmetry $\mu=0$, we have the finite-temperature self-consistent equation
\[
...
The above sections offer a self-consistency equation for the only HFP $m$; $W$ is determined by $\Delta = mU$ indeed. In an iterative HF fashion, Eq.~\eqref{appeq:hm-af-m-self-consistency} reads:
\begin{equation}\label{appeq:hm-af-m-self-consistency-iterative}
	m_{i+1} =
...
\begin{enumerate}
	...
	\end{enumerate}
	\item Compute $m_{i+1}$ using Eq.~\eqref{appeq:hm-af-m-self-consistency-iterative} with chemical potential $\mu_i$ and update the counter, $i \to i+1$;

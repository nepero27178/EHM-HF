...
The Hubbard model defined on the planar square lattice presents three core symmetries, discussed in detail by \citeauthor{arovas2022hubbard} \cite{arovas2022hubbard}: translational invariance, spin rotational invariance and charge conservation. In this appendix we investigate magnetic ordering: thus we perform a $\mathrm{SU}(2)\to\mathrm{U}(1)$ SSB of the kind depicted in Fig.~\ref{fig:su2-u1-domains}, choosing a particular magnetization direction.

As is done in Chap.~\ref{chap:mft-discussion}, the starting point is to apply Wick's theorem to reduce the quartic hamiltonian of Eq.~\eqref{appeq:hubbard-model} to a quadratic form. Considering the discussion of Sec.~\ref{subsec-mft-treatment-hm}, we need to reduce the quartic interaction down to a quadratic operator, and from Tab.~\ref{tab:U-wick-terms-phases} we know this is only possible for the pure Hubbard model $\hat H_t + \hat H_U$ in the Hartree channel,
\[
\begin{aligned}
...
\end{aligned}
\]
where $\delta \hat n_{i\sigma} \equiv \hat n_{i\sigma} - \ev{\hat n_{i\sigma}}$ and orders higher than first have been ignored, assuming negligible fluctuations around the equilibrium single-site population. The first term of the above three can be neglected at fixed particles number, being a pure energy shift. Cooper fluctuations are in principle possible for the pure Hubbard model, and that is dealt with in Sec.~\mref. For now, let us focus on the realization of the AF phase.
...
We work in a commensurate AF phase ordering, which is, we explicitly break symmetry by realizing a SDW phase with spin density unevenly distributed on sites with periodicity of two sites, each direction. Let us change notation, indicating each site as
\[
	i \to \mathbf{r} = (x,y)
	\qquad
	x,y \in \mathbb{N}
...
\begin{align}
	...
	\right] - mU \sum_\mathbf{r} (-1)^{x+y} \left[
		\hat n_{\mathbf{r}\uparrow} - \hat n_{\mathbf{r}\downarrow}
	\right] - E_{\mathrm{H}/U}^{(\mathrm{AF})} \label{appeq:hubbard-mean-field-hamiltonian}
\end{align}
...
\begin{align}
	...
	&= L_x L_y \times U \left(
		n^2 - m^2
	\right) \label{appeq:af-hartree-V-contraction-energy}
\end{align}
By a Fourier transform, the phase factor of Eq.~\eqref{appeq:hubbard-mean-field-hamiltonian} can be absorbed in the destruction operator inside of $\hat n_{\mathbf{r}\sigma}$:
\[
\begin{aligned}
...
\begin{equation}\label{appeq:hubbard-bogoliubov-hamiltonian-pseudofields}
	...
	\end{bmatrix}
\end{equation}
The hamiltonian represents a system of spins subject to local magnetic fields, each tilted by an angle $\tan(2\theta_{\mathbf{k}\sigma}) = \Delta_{\mathbf{k}\sigma}/\epsilon_{\mathbf{k}\sigma}$, as sketched in Fig.~\ref{appfig:pseudo-magnetic-field}. At any finite temperature, the ground-state of such a system is well-known: a thermal superposition of ``down'' spins with ``up'' spins. In the pseudospin picture, at each ``site'' $(\mathbf{k}\sigma)$, the lowest-energy state is the one anti-aligned with the pseudo-magnetic field. It is useful now to define the thermal factors,
\begin{equation}\label{appeq:af-thermal-factors-def}
	\mathrm{Th}_{\mathbf{k}\sigma}^{(\pm)}[\beta,\mu] \equiv 
	f\left(
		E_{\mathbf{k}\sigma};\beta,\mu
	\right) \pm f\left(
		-E_{\mathbf{k}\sigma};\beta,
	\right)
\end{equation}
that will appear quite often. Then it's immediate to derive the various expectation values.
\begin{align}
	\ev{\hat s_{\mathbf{k}\sigma}^x} &= -\frac{\Re\lbrace\Delta_{\mathbf{k}\sigma}\rbrace}{2E_{\mathbf{k}\sigma}} \mathrm{Th}_{\mathbf{k}\sigma}^{(-)}[\beta,\mu] \label{appeq:hm-af-sx}\\
	\ev{\hat s_{\mathbf{k}\sigma}^y} &= -\frac{\Im\lbrace\Delta_{\mathbf{k}\sigma}\rbrace}{2E_{\mathbf{k}\sigma}}  \mathrm{Th}_{\mathbf{k}\sigma}^{(-)}[\beta,\mu] \label{appeq:hm-af-sy}\\
	\ev{\hat s_{\mathbf{k}\sigma}^z} &=  \frac{\epsilon_{\mathbf{k}\sigma}}{2E_{\mathbf{k}\sigma}}  \mathrm{Th}_{\mathbf{k}\sigma}^{(-)}[\beta,\mu] \label{appeq:hm-af-sz}
\end{align}
...
\[
	...
	\end{bmatrix}
	\qq{and}
	W_{\mathbf{k}\sigma} \equiv e^{-i \theta_{\mathbf{k}\sigma} 
	\tau^y} = \begin{bmatrix}
...
\end{aligned}
\]
In the last passage, Eq.~\eqref{appeq:mixed-cc-term-intermediate-passage} has been used. By multiplying the above equation by $U$, we get the HF self-consistency equation for the HFP $\Delta=mU$,
\begin{align}
	\Delta &= \frac{U}{2L_xL_y} \sum_{\mathbf{k} \in \mathrm{MBZ}} \left[
		\langle
		...
		\rangle
		 ...
		 \rangle
	\right] \nonumber \\
	&= -\frac{U}{2L_xL_y} \sum_{\mathbf{k} \in \mathrm{MBZ}} \frac{\Re\lbrace\Delta_{\mathbf{k}\uparrow}\rbrace-\Re\lbrace\Delta_{\mathbf{k}\downarrow}\rbrace}{2E_\mathbf{k}} \mathrm{Th}_\mathbf{k}^{(-)}[\beta,\mu] \nonumber \\
	&= -\frac{U}{2L_x L_y} \sum_{\mathbf{k} \in \mathrm{MBZ}} \frac{\Delta}{2E_\mathbf{k}} \mathrm{Th}_\mathbf{k}^{(-)}[\beta,\mu] \label{appeq:hm-af-Delta-self-consistency}
\end{align}
Of course, the same equation would have been obtained more straightforwardly by the equivalent definition of $\Delta_{\mathbf{k}\sigma}$,
\[
	\Delta_{\mathbf{k}\sigma} \equiv \frac{U}{L_x L_y} \sum_{\mathbf{k},\mathbf{k}'} 
	\langle
...
The above sections offer a self-consistency equation for the only HFP $\Delta$; $W$ is determined by $\Delta$ indeed. In an iterative HF fashion, Eq.~\eqref{appeq:hm-af-Delta-self-consistency} reads:
\begin{equation}\label{appeq:hm-af-Delta-self-consistency-iterative}
	\Delta_{i+1} =
	\frac{U}{2L_x L_y} \sum_{\mathbf{k} \in \mathrm{MBZ}} \frac{\Delta_i}{2E_\mathbf{k}[\Delta_i]} \left[
	f(-E_\mathbf{k}[\Delta_i];\beta,\mu)-f(E_\mathbf{k}[\Delta_i];\beta,\mu)
	\right]
\end{equation}
Then a self-consistent algorithm to search for a self consistent estimate for $m = \Delta/U$ may be sketched, following the general idea of Sec.~\ref{sec:computational-mft}:
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item the tolerance parameters $\delta_\Delta, \Delta_n \in \mathbb{R}$ (respectively for the HFP $\Delta$ and density);
	\end{itemize}
	\item Select a random starting value $\Delta_i > 0$ for $i=1$;
	...
	\begin{enumerate}
		...
		\[
			\delta n(\mu; \beta, \Delta_i, U) \equiv \frac{1}{L_x L_y} \sum_{\mathbf{k} \in \mathrm{MBZ}} \left[
			f\left(-E_\mathbf{k}[U,\Delta_i]; \beta,\mu \right) + f \left( E_\mathbf{k}[U,\Delta_i]; \beta,\mu \right)
			\right] - n
		\]
		...
		\[
			\delta n(\mu_i) = 0
		\]
		Then $\mu_i$ is the chemical potential which, at step $i$ with HFP $\Delta_i$, realizes the best approximation of density $n$;
	\end{enumerate}
	\item Compute $\Delta_{i+1}$ using Eq.~\eqref{appeq:hm-af-Delta-self-consistency} with chemical potential $\mu_i$ and update the counter, $i \to i+1$;
	\item Check if $\abs{\Delta_{i+1}-\Delta_i} \le \delta_\Delta$:
	\begin{itemize}
		\item If yes, halt;
		...
		\begin{itemize}
			...
			\item If not, define 
			\[
				\Delta_{i+1} \colon g \Delta_{i+1} + (1-g) \Delta_i
			\]
...
\section{(Impossibility of a stable) superconducting phase}